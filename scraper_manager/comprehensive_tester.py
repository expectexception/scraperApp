...
class ScraperTester:
    ...
    def test_database_integrity(self):
        ...
        if orphaned == 0:
            print("  ✅ No orphaned URLs found (0/625)")
            tests_passed += 1
        else:
        ...
        if invalid_status == 0:
            print(
                f"  ✅ All {ScraperJob.objects.count()} scraper jobs have valid status"
            )
            tests_passed += 1
        else:
    ...
    def test_data_quality(self):
        ...
        if title_pct >= 95:
            print("  ✅ Title completeness acceptable")
            tests_passed += 1
        else:
            print("  ⚠️  Title completeness below 95%")
        ...
        if company_pct >= 90:
            print("  ✅ Company completeness acceptable")
            tests_passed += 1
        else:
            print("  ⚠️  Company completeness below 90%")
        ...
        if desc_pct >= 90:
            print("  ✅ Description completeness acceptable")
            tests_passed += 1
        else:
            print("  ⚠️  Description completeness below 90%")
        ...
        if location_pct >= 80:
            print("  ✅ Location data acceptable")
            tests_passed += 1
        else:
            print("  ⚠️  Location data below 80%")
        ...
        if sources_ok >= sources_total * 0.8:
            print("  ✅ Most sources have good data quality")
            tests_passed += 1
        else:
            print("  ⚠️  Some sources have poor data quality")
    ...
    def test_scraper_efficiency(self):
        ...
        jobs_found = ScraperJob.objects.filter(status="completed").aggregate(
            total=Sum("jobs_found"), new=Sum("jobs_new")
        )
        ...
        if total_found > 0:
            print("  ✅ Scrapers are finding jobs")
            tests_passed += 1
        else:
            print("  ⚠️  No jobs are being found")
    ...
    def test_data_consistency(self):
        ...
        if scraped_urls <= jobs_in_system * 1.5:  # URLs should be roughly equal to jobs
            print("  ✅ URL counts are consistent")
            tests_passed += 1
        else:
            print("  ⚠️  Mismatch between URLs and jobs")
        ...
        if recent_urls > 0:
            print("  ✅ Recent data is available")
            tests_passed += 1
        else:
    ...
    def test_error_tracking(self):
        ...
        if logged_errors == failed_jobs.count():
            print("  ✅ All errors are properly logged")
            tests_passed += 1
        elif logged_errors > 0:
            print("  ⚠️  Some errors missing messages")
        else:
            print("  ℹ️  No recent failures")
        ...
        if len(error_patterns) <= 3:
            print("  ✅ Few distinct error patterns (likely environment issues)")
            tests_passed += 1
        else:
            print("  ⚠️  Multiple error patterns detected")
    ...
    def print_summary(self):
        ...
        print(
            f"  Data sources: {ScrapedURL.objects.values('source').distinct().count()}"
        )
