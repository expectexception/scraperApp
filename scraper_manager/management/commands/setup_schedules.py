...
class Command(BaseCommand):
    ...
    def clear_schedules(self):
        ...
        deleted = PeriodicTask.objects.filter(name__startswith="scraper_").delete()
        self.stdout.write(
            self.style.WARNING(f"Cleared {deleted[0]} existing scraper schedules")
        )

    def disable_all_schedules(self):
        """Disable all scraper schedules"""
        updated = PeriodicTask.objects.filter(name__startswith="scraper_").update(
            enabled=False
        )
    ...
    def setup_default_schedules(self):
        ...
        if created:
            count_created += 1
            self.stdout.write("  ✓ Created: scraper_run_all_every_3_hours")
        else:
            count_updated += 1
            self.stdout.write("  ↻ Updated: scraper_run_all_every_3_hours")
        ...
        if created:
            count_created += 1
            self.stdout.write("  ✓ Created: scraper_cleanup_weekly")
        else:
            count_updated += 1
            self.stdout.write("  ↻ Updated: scraper_cleanup_weekly")
        ...
        self.stdout.write(
            self.style.WARNING("⚠️  Remember to start Celery Beat worker:\n")
        )
