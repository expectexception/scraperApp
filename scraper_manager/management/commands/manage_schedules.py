...
class Command(BaseCommand):
    ...
    def view_schedules(self):
        ...
        self.stdout.write(
            f"Global Auto-Schedule: {'✅ ENABLED' if AUTO_SCHEDULE.get('enabled') else '❌ DISABLED'}\n"
        )
        ...
        for key, desc in schedules:
            ...
            if "max_jobs" in schedule:
                self.stdout.write(
                    f"   Max Jobs: {schedule.get('max_jobs', 'unlimited')}"
                )
    ...
    def update_schedule(self, schedule_name, cron):
        ...
        self.stdout.write(
            f'Set: AUTO_SCHEDULE["{schedule_name}"]["schedule"] = "{cron}"'
        )
        self.stdout.write("Then restart Celery: celery -A backendMain worker -l info")
