...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--source",
            type=str,
            help="Source to process (e.g., indigo, aviationjobsearch)",
        )
        parser.add_argument(
            "--limit",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        qs = Job.objects.filter(
            source__iexact=source, posted_date__isnull=True
        ).order_by("-created_at")
        ...
        if not jobs:
            self.stdout.write(
                self.style.SUCCESS(f"No jobs for {source} need backfilling.")
            )
            return

        self.stdout.write(
            f"Processing {total} jobs from source={source} (dry_run={dry})..."
        )
                                ...
                                for item in items:
                                    if isinstance(item, dict) and (
                                        ...
                                        if item.get("datePosted"):
                                            parsed = scraper.parse_posted_date(
                                                item.get("datePosted")
                                            )
                                            if parsed:
                                                parsed_date = parsed
