...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for job in qs:
            needs_update = (
                not job.location
                or job.location == ""
                or job.location == "Unknown"
                or not job.posted_date
            )
            if needs_update:
        ...
        async def process_jobs():
            ...
            for i, job in enumerate(jobs):
                try:
                    self.stdout.write(
                        f"[{i + 1}/{total}] Processing: {job.title[:50]}..."
                    )
                    ...
                    if jobs_with_updates and len(jobs_with_updates) > 0:
                        ...
                        changes = []
                        if (
                            updated_job.get("location")
                            and updated_job["location"] != job.location
                        ):
                            changes.append(
                                f"location: '{job.location}' -> '{updated_job['location']}'"
                            )
                        if updated_job.get("posted_date") and str(
                            updated_job["posted_date"]
                        ) != str(job.posted_date):
                            changes.append(
                                f"posted_date: '{job.posted_date}' -> '{updated_job['posted_date']}'"
                            )
                        if updated_job.get("description") and len(
                            updated_job["description"]
                        ) > len(job.description or ""):
                            changes.append(
                                f"description: {len(job.description or '')} -> {len(updated_job['description'])} chars"
                            )
        ...
        try:
            ...
            if dry:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"DRY RUN: Would update {updated} jobs, {errors} errors"
                    )
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(f"Updated {updated} AAP jobs, {errors} errors")
                )
    ...
    def _update_job(self, job, updated_job):
        ...
        if updated_job.get("description") and len(updated_job["description"]) > len(
            job.description or ""
        ):
