...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            try:
                ...
                if dry_run:
                    if created:
                        self.stdout.write(
                            self.style.WARNING(
                                f"  [DRY RUN] Would create: {scraper_name}"
                            )
                        )
                        created_count += 1
                    else:
                        self.stdout.write(
                            self.style.NOTICE(
                                f"  [DRY RUN] Already exists: {scraper_name}"
                            )
                        )
                        skipped_count += 1
                else:
                    if created:
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"  ✓ Created config for: {scraper_name}"
                            )
                        )
                        created_count += 1
                    else:
        ...
        if created_count > 0 and not dry_run:
            ...
            self.stdout.write("\nNext steps:")
            self.stdout.write(
                "  1. Go to admin panel: /admin/scraper_manager/scraperconfig/"
            )
