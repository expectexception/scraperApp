...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        qs = Job.objects.filter(
            source__iexact="aviationjobsearch", posted_date__isnull=True
        ).order_by("-created_at")
        ...
        if not jobs:
            self.stdout.write(
                self.style.SUCCESS(
                    "No Aviation Job Search jobs found that need backfilling."
                )
            )
            return

        self.stdout.write(
            f"Processing {total} Aviation Job Search jobs (dry_run={dry})..."
        )
