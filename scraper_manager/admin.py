...
class ScraperJobAdmin(admin.ModelAdmin):
    ...
    def trigger_scraper_view(self, request):
        ...
        if request.method == 'POST' or (request.method == 'GET' and request.GET.get('immediate') == 'true'):
            # Get params from POST or GET
            scraper_name = request.POST.get('scraper_name') or request.GET.get('scraper_name')
    ...
    def run_selected_scrapers(self, request, queryset):
        ...
        for config in queryset.filter(is_enabled=True):
            ...
            thread = threading.Thread(
                target=run_scraper, 
                args=(config.scraper_name, config.max_jobs)
            )
            thread.daemon = True
            thread.start()
            count += 1
...
class ScrapedURLAdmin(admin.ModelAdmin):
            ...
            if CompanyMapping:
                try:
                    ...
                    try:
                        ...
                        return format_html('<a href="{}" style="color: #0066cc;">🏢 {}</a>', url, obj.company)
                    except Exception:
                    ...
                    logger.exception("Error looking up CompanyMapping for %s: %s", obj.company, str(e))
                    return obj.company
            else:
                return obj.company
        return "—"
    ...
    def mark_active(self, request, queryset):
        ...
        self.message_user(request, f"✅ Marked {updated} URLs as active.", messages.SUCCESS)
    ...
    def verify_url_activity(self, request, queryset):
        ...
        if queryset.count() > 20:
            self.message_user(request, "⚠️ Please select 20 or fewer URLs for manual verification.", level='warning')
            return
        ...
        try:
            ...
            inactive_count = 0
            for obj in queryset:
                is_active, reason = loop.run_until_complete(db.check_job_active(obj.url))
                if not is_active:
                    obj.is_active = False
            ...
            loop.close()
            self.message_user(request, f"✅ Verification complete. {inactive_count} URL(s) marked as inactive.")
        except Exception as e:
            import logging
            logger = logging.getLogger('scraper_manager')
            logger.error(f"URL Verification Error: {e}", exc_info=True)
            self.message_user(request, f"❌ Verification error: {str(e)}", level='error')
