    ...
    try:
        ...
        for scraper_name in sorted(SCRAPERS.keys()):
            display_name = display_names.get(
                scraper_name, scraper_name.replace("_", " ").title()
            )
...
class ScraperJob(models.Model):
    ...
    task_id = models.CharField(
        max_length=255, blank=True, db_index=True, help_text="Celery task ID"
    )
    worker_name = models.CharField(
        max_length=255, blank=True, help_text="Worker hostname"
    )
    pid = models.IntegerField(null=True, blank=True, help_text="OS Process ID")
    progress = models.IntegerField(default=0, help_text="Progress percentage 0-100")
    progress_message = models.CharField(
        max_length=255, blank=True, help_text="Latest progress detail"
    )
...
class ScraperConfig(models.Model):
    ...
    retry_count = models.IntegerField(
        default=3, help_text="Number of retries on failure"
    )
    ...
    def update_stats(self, success=True):
        ...
        self.save(
            update_fields=["total_runs", "successful_runs", "failed_runs", "last_run"]
        )
