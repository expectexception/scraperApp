...
class SkyWestScraper(BaseScraper):
    """
    Scraper for SkyWest Airlines (iCIMS)
    """

    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key="skywest", db_manager=db_manager)
        self.base_url = "https://skywest-airlines.icims.com/jobs/search?in_iframe=1"
                ...
                for row in job_rows:
                    ...
                    try:
                        ...
                        if url and not url.startswith("http"):
                            url = (
                                "https://"
                                + "https://skywest-airlines.icims.com/jobs/search?in_iframe=1".split(
                                    "/"
                                )[2]
                                + url
                            )
