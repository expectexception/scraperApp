                ...
                while len(jobs) < self.max_jobs:
                    ...
                    try:
                        await page.wait_for_selector(
                            "h2 a, .job-title a", timeout=10000
                        )
                    except Exception:
                        logger.warning(
                    ...
                    for link in job_links:
                        ...
                        if (
                            ...
                            and self.is_job_link(text, href)
                        ):
                            seen_urls.add(href)
                            unique_links.append(link)

                    logger.info(
                        f"[{self.site_key}] Found {len(unique_links)} potential jobs on page"
                    )
                    ...
                    for link in unique_links:
                        ...
                        try:
                            ...
                            await link.evaluate_handle(
                                'el => el.closest("div")'
                            )  # Approximate, might need tuning
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    desc_el = await detail_page.query_selector(
                                        ".job-details, .job-content, .vacancy-desc"
                                    )
                                    if desc_el:
                                    ...
                                    logger.warning(
                                        f"[{self.site_key}] Description extraction error: {e}"
                                    )
                                    description = (
                                        await self.extract_description_from_page(
                                            detail_page
                                ...
                                try:
                                    ...
                                    apply_btn = await detail_page.query_selector(
                                        'a:has-text("Apply now"), a.apply-link, a[href*="vacancy-application"]'
                                    )
                                    if apply_btn:
                                        raw_apply = await apply_btn.get_attribute(
                                            "href"
                                        )
                                        if raw_apply:
                                            ...
                                            if not raw_apply.startswith("http"):
                                                ...
                                                apply_url = raw_apply
                                            logger.info(
                                                f"[{self.site_key}] Found Apply URL: {apply_url}"
                                            )
                                except Exception as e:
                                    logger.warning(
                                        f"[{self.site_key}] Apply link extraction error: {e}"
                                    )
                                ...
                                logger.warning(
                                    f"[{self.site_key}] Failed to process detail {url}: {e}"
                                )
                            finally:
                            ...
                            logger.error(
                                f"[{self.site_key}] Error extracting job link: {e}"
                            )

                    if jobs_on_page == 0:
                        logger.info(
                            f"[{self.site_key}] No valid new jobs found on page {page_count}"
                        )
                        break
