...
class JetBlueScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.jobs_url = self.site_config.get('jobs_url', 'https://careers.jetblue.com/search/?createNewAlert=false&q=&locationsearch=')

    async def run(self):
        """Main execution method"""
        self.print_header()

        print(f"Fetching jobs from {self.site_config.get('name', 'JetBlue Airways')}...")
                ...
                for start_row in range(0, 500, 25): # 25 results per page
                    ...
                    for row in rows:
                        try:
                            ...
                            req_id = (await req_elem.inner_text()).strip() if req_elem else None
