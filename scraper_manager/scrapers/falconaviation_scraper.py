...
class FalconAviationScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.jobs_url = self.site_config.get('jobs_url', 'https://falconaviation.ae/careers')
    ...
    async def fetch_jobs_from_listing(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    await page.wait_for_selector('button.accordion-button', timeout=20000)
                except Exception:
                    logger.warning("Timed out waiting for accordion-button on Falcon Aviation")
                ...
                logger.info(f"Found {len(job_links)} accordion jobs using verified selector: button.accordion-button")

                if not job_links:
                    logger.warning("No jobs found using the verified accordion button selector.")
                    return []
                ...
                for link in job_links:
                    ...
                    try:
                        ...
                        skip_words = {
                            'apply now', 'log in', 'login', 'careers', 'home', 'search',
                            'sign up', 'register', 'read more', 'view details', 'select',
                            'all open positions', 'general application'
                        }
