        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for p_num in range(1, self.max_pages + 1):
                    ...
                    self.logger.info(
                        f"✓ Found {found_on_page} jobs for '{keyword}' on page {p_num}"
                    )

                    if found_on_page == 0 or (
                        self.max_jobs and len(jobs) >= self.max_jobs
                    ):
                        break
