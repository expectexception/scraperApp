            ...
            try:
                ...
                try:
                    await page.wait_for_selector(
                        "table#searchresults tr.data-row", timeout=20000
                    )
                except Exception:
                    logger.warning(
                        f"[{self.site_key}] Timeout waiting for job rows. Checking if any jobs are present."
                    )
                ...
                logger.info(
                    f"[{self.site_key}] Found {len(job_rows)} potential job rows in listing"
                )

                initial_jobs = []
                for row in job_rows:
                    title_elem = await row.query_selector(
                        "td.colTitle span.jobTitle a.jobTitle-link"
                    )
                    loc_elem = await row.query_selector(
                    ...
                    if title_elem:
                        ...
                        posted_date_raw = (
                            (await date_elem.inner_text()).strip() if date_elem else ""
                        )
                ...
                for i, j_initial in enumerate(matched_initial):
                    ...
                    try:
                        ...
                        if posted_date_raw:
                            ...
                            posted_date = (
                                parsed_date if parsed_date else posted_date_raw
                            )
