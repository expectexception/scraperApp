...
class CarpatairScraper(BaseScraper):
    """
    Scraper for Carpatair
    URL: https://www.carpatair.com/careers/
    """
            ...
            try:
                ...
                links = await page.evaluate('''() => {
                    return Array.from(document.querySelectorAll('a'))
                        ...
                        .filter(a => a.t && a.t.length > 5 && a.h.includes('career'))
                }''')
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        desc_loc = detail_page.locator('main, article, .job-description, .content, #content')
                        for loc in ['main', 'article', '.job-description', '.content', '#content']:
                        ...
                        job_id = f"carpatair_{i+1}"
                        match = re.search(r'career/([^/]+)', url)
                        if match:
                            job_id = f"carpatair_{match.group(1)}"
