...
class RoyalJetScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.base_url = self.site_config.get('base_url', 'https://careerroyaljet.talentera.com')
        self.jobs_url = self.site_config.get('jobs_url', 'https://careerroyaljet.talentera.com/en/job-search-results/')
    ...
    async def fetch_jobs_from_listing(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(f"Found {len(job_links)} jobs using verified selector: a.text-snip")

                if not job_links:
                    logger.warning("No job links found with verified selector for Royal Jet.")
                ...
                for link in job_links:
                    ...
                    try:
                        ...
                        skip_words = {'apply now', 'log in', 'login', 'careers', 'home', 'search', 'sign up', 'register', 'jobs'}
                        ...
                        job_data = {
                            ...
                            'url': job_url,
                            'apply_url': job_url,
                            'location': 'Abu Dhabi, UAE', # Typically Royal Jet
                            'job_type': '',
                            'department': '',
                            'posted_date': '',
                            'closing_date': '',
                        ...
                        parent_handle = await link.evaluate_handle('el => el.closest("tr") || el.closest("li") || el.closest(".job")')
