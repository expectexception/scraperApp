        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        cookie_btn = page.locator('.cc-btn.cc-allow-all') # Common cookie banner class, double check if possible, or just click coordinates
                        # Based on analysis, accepting cookies might be needed. 
                        # Using a generic wait for now as we didn't see explicit cookie blocking in subagent (it clicked coordinates)
                        # Let's try to click body to dismiss overlays if any
                    ...
                    try:
                        ...
                        logger.warning(f"[{self.site_key}] Job cards not found after waiting.")
                ...
                while True:
                    ...
                    if len(job_cards) == job_count_prev:
                        ...
                        if retries > 3: # No new jobs after 3 scrolls
                            logger.info(f"[{self.site_key}] Reached end of list or no new jobs loading.")
                            break
                    else:
                    ...
                    # Wait for load
                    try:
                        await page.wait_for_timeout(2000) # Fixed wait for animation/fetch
                    ...
                    if job_count_prev > 500: 
                        logger.info(f"[{self.site_key}] Reached safety limit of 500 jobs.")
                        break
