    ...
    async def fetch_jobs(self) -> List[Dict[str, Any]]:
        ...
        async with async_playwright() as p:
            ...
            async def handle_response(response):
                if (
                    "rec-job-search/external/jobs" in response.url
                    and response.request.method == "POST"
                ):
                    try:
                        data = await response.json()
                        logger.info(
                            f"[{self.site_key}] Intercepted payload keys: {list(data.keys()) if data else 'None'}"
                        )
                        if data and "data" in data:
                            jobs_list = data["data"].get("jobs", [])
                            logger.info(
                                f"[{self.site_key}] Jobs in 'data.jobs': {len(jobs_list)}"
                            )
                            captured_jobs.extend(jobs_list)
                        elif data and "jobs" in data:
                            captured_jobs.extend(data["jobs"])
                            logger.info(
                                f"[{self.site_key}] Jobs in 'jobs': {len(data['jobs'])}"
                            )
                        else:
                            logger.warning(
                                f"[{self.site_key}] Unexpected JSON structure: {str(data)[:200]}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"[{self.site_key}] Error parsing API response: {e}"
                        )
            ...
            try:
                ...
                if not captured_jobs:
                    logger.warning(
                        f"[{self.site_key}] No jobs captured via API, triggered search..."
                    )
                    try:
                        ...
                        if search_input:
                            ...
                            logger.info(
                                f"[{self.site_key}] Triggered search with space"
                            )
                            await page.wait_for_load_state("networkidle")
                            await asyncio.sleep(5)
                    except Exception as e:
                        logger.warning(
                            f"[{self.site_key}] Error triggering search: {e}"
                        )
                ...
                if not captured_jobs:
                    ...
                    try:
                        await page.wait_for_selector(
                            'li[data-requisition-id], div[data-requisition-id], a[href*="/requisition/"]',
                            timeout=5000,
                        )
                    except:
                        pass
                    ...
                    job_cards = await page.query_selector_all(
                        "li[data-requisition-id], div[data-requisition-id]"
                    )
                    if not job_cards:
                        ...
                        for card in job_cards:
                            ...
                            if not loc_el:
                                ...
                                loc_el = await card.query_selector(
                                    "div:not(:has(h3,h4)), span:not(:has(a))"
                                )
                    ...
                    logger.info(
                        f"[{self.site_key}] DOM Fallback found {len(captured_jobs)} jobs"
                    )
                ...
                while len(jobs) < self.max_jobs and no_new_jobs_count < 3:
                    current_batch = [
                        ...
                        if j.get("requisitionId") not in processed_ids
                    ]
                        ...
                        if not any(j.get("fallback") for j in captured_jobs):
                            try:
                                ...
                                if next_btn and await next_btn.is_enabled():
                                    logger.info(
                                        f"[{self.site_key}] navigating to next page..."
                                    )
                    ...
                    for job_entry in current_batch:
                        ...
                        try:
                            ...
                            if (
                                ...
                                try:
                                    logger.info(
                                        f"[{self.site_key}] Navigating to detail: {url}"
                                    )
                                    ...
                                    try:
                                        await detail_page.wait_for_selector(
                                            '.cs-at-job-description, div[data-name="jobDescription"], .job-description',
                                            timeout=10000,
                                        )
                                    except:
                                        logger.debug(
                                            f"[{self.site_key}] Timeout waiting for specific description selector"
                                        )

                                    desc_el = await detail_page.query_selector(
                                        '.cs-at-job-description, div[data-name="jobDescription"], .job-description, .c-job-description-details'
                                    )
                                    if desc_el:
                                        description = await desc_el.inner_text()
                                    else:
                                        description = (
                                            await self.extract_description_from_page(
                                                detail_page
                                            )
                                        )
                                    ...
                                    if not location or location == "Unknown":
                                        try:
                                            ...
                                            for sel in loc_selectors:
                                                loc_el = (
                                                    await detail_page.query_selector(
                                                        sel
                                                    )
                                                )
                                                if loc_el:
                                    ...
                                    logger.warning(
                                        f"[{self.site_key}] Failed to get detail: {e}"
                                    )
                                finally:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-jobs", type=int, default=10, help="Maximum jobs to scrape"
    )
    parser.add_argument("--verbose", action="store_true", help="Verbose mapping")
    args = parser.parse_args()

    config = {"scrapers": {"iata": {"max_jobs": args.max_jobs}}}
