    ...
    async def fetch_jobs(self) -> List[Dict[str, Any]]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                search_combinations = [(q, l) for q in search_queries for l in search_locations]
                ...
                for query, loc in search_combinations:
                    ...
                    try:
                        ...
                        while len(jobs) < self.max_jobs:
                            ...
                            logger.info(f"[{self.site_key}] Found {len(job_elements)} jobs on page {page_count+1}")
                        ...
                        logger.warning(f"[{self.site_key}] Search combination failed: {e}")
                        continue
        ...
        async with async_playwright() as p:
            ...
            for job in jobs:
                ...
                try:
                    ...
                    desc_el = await page.query_selector('div.ats-description') or await page.query_selector('main')
                    job['description'] = await desc_el.inner_html() if desc_el else ""
                except Exception as e:
                    logger.warning(f"[{self.site_key}] Failed to load description for {job['url']}: {e}")
                finally:
                    await page.close()
