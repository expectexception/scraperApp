...
class AirBalticScraper(BaseScraper):
    """
    Scraper for Air Baltic.
    URL: https://careers.airbaltic.com/en/jobs
    """
    
    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key='airbaltic', db_manager=db_manager)
        self.base_url = "https://careers.airbaltic.com/en/jobs"
        self.company_name = "Air Baltic"
    ...
    async def fetch_jobs(self) -> list:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    cookie_btn = page.locator('text=Accept all, text=Agree').first
                ...
                links = await page.evaluate('''() => {
                    return Array.from(document.querySelectorAll('a'))
                        ...
                        .filter(a => a.t && a.t.length > 5 && (a.h.includes('/jobs/') || a.h.includes('/job')))
                }''')
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        for loc in ['.jobdescription', '.job-description', 'main', '.description-content', '.job-details']:
                        ...
                        job_id = f"airbaltic_{i+1}"
                        match = re.search(r'jobs/([^/]+)', url)
                        if match and match.group(1):
                            job_id = f"airbaltic_{match.group(1).replace('-', '_')}"
