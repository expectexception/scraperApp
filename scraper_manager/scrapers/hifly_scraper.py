...
class HiFlyScraper(BaseScraper):
    """
    Scraper for Hi Fly
    URL: https://hifly.aero/careers/
    """

    def __init__(self, config: Dict, db_manager=None):
        super().__init__(config, site_key="hifly", db_manager=db_manager)
        self.base_url = "https://hifly.aero"
        self.jobs_url = "https://hifly.aero/careers/"
        self.company_name = "Hi Fly"
    ...
    async def fetch_jobs(self) -> List[Dict]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Look for links inside job list
                job_elements = await page.query_selector_all("a[href*='factorialhr.pt/apply/']")
                ...
                for el in job_elements:
                    ...
                    title = await el.evaluate("(node) => { const parent = node.closest('.careers-list-header-wrapper') || node.closest('li') || node.parentElement.parentElement; const h2 = parent.querySelector('h2'); return h2 ? h2.innerText : ''; }")
                    title = title.strip() if title else "Unknown"
                    if not title or title.lower() in ["read more", "apply", "details", "careers"]:
                        continue
                    ...
                    jobs.append({
                        ...
                        "title": title,
                        "location": "Portugal", # Default for Hi Fly
                        "url": full_url,
                        "source_url": full_url,
                        "apply_url": full_url,
                        "is_active": True,
