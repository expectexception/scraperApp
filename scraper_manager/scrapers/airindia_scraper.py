...
class AirIndiaScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.jobs_url = self.site_config.get(
            "jobs_url",
            "https://careers.airindia.com/sfcareer/jobreqcareer?jobPipeline=AirIndia",
        )
    ...
    async def run(self):
        ...
        print(
            f"Fetching jobs from {self.site_config.get('name', 'Air India Careers')}..."
        )
    ...
    async def fetch_jobs_from_listing(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                print(
                    f"✓ Found {len(job_links)} jobs using verified selector: a.jobTitle-link"
                )

                if not job_links:
                    print(
                        "❌ No job links found — SuccessFactors page may not have rendered"
                    )
                    return jobs
    ...
    async def _extract_description(self, browser, job):
        ...
        try:
            ...
            if page_title and "Air India" in page_title:
                # Clean up title
                title_clean = (
                    page_title.replace(" - Air India Careers", "")
                    .replace(" | Air India", "")
                    .replace("Job Details Careers", "")
                    .strip()
                )
