...
class FinnairScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.base_url = (
            "https://finnair.wd103.myworkdayjobs.com/wday/cxs/finnair/finnair/jobs"
        )
    ...
    async def fetch_jobs(self) -> list:
        ...
        try:
            ...
            logger.error(
                f"[{self.site_key}] curl_cffi not installed, cannot bypass WAF."
            )
            return jobs
        ...
        while True:
            try:
                logger.info(
                    f"[{self.site_key}] Fetching jobs from Workday ATS API (offset {payload['offset']})"
                )
                ...
                if response.status_code != 200:
                    logger.error(
                        f"[{self.site_key}] Target returned status {response.status_code}"
                    )
                    break
                ...
                try:
                    ...
                    logger.error(
                        f"[{self.site_key}] JSON parse failed. Body: {response.text[:500]}"
                    )
                    break
                if not isinstance(data, dict):
                    logger.error(
                        f"[{self.site_key}] Expected dict but got {type(data)}: {str(data)[:200]}"
                    )
                    break
                ...
                for item in job_list:
                    ...
                    job_id = (
                        f"finnair_{job_req_id}"
                        if job_req_id
                        else f"finnair_{externalPath.split('/')[-1]}"
                    )
                ...
                logger.error(
                    f"[{self.site_key}] Error fetching jobs from API: {e}\n{traceback.format_exc()}"
                )
                break
    ...
    async def _fetch_job_description(self, job):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(
                    f"[{self.site_key}] Failed to fetch description for {job['job_id']} (Status {response.status_code})"
                )
        except Exception as e:
            self.logger.error(
