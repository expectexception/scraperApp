...
class MountainAirCargoScraper(BaseScraper):
    ...
    async def fetch_jobs(self) -> List[Dict]:
        ...
        try:
            ...
            def fetch_url():
                return requests.get(self.base_url, headers={
            ...
            if not html_content or response.status_code != 200:
                logger.error(f"[{self.site_key}] Failed to retrieve HTML: Status {response.status_code}")
                return jobs
            ...
            for a in soup.find_all('a', href=True):
                if 'job-opening.php' in a['href']:
                    title = a.text.strip()
                    if title and title not in ['Department', 'Position Title', 'City', 'State']:
            ...
            for title, job_url in job_links:
                ...
                def fetch_detail():
                    return requests.get(job_url, headers={
                        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
                    }, timeout=20.0)
                    
                detail_resp = await asyncio.to_thread(fetch_detail)
                if detail_resp.status_code != 200:
                    logger.warning(f"[{self.site_key}] Failed to fetch details for {title}")
                    continue
                ...
                location = "Unknown"
                loc_tag = detail_soup.find('div', class_='job_location') or detail_soup.find('span', class_='location')
                if loc_tag:
                    location = loc_tag.text.strip()
                
                # Description
                desc_tag = detail_soup.find('div', id='job_description') or detail_soup.find('div', class_='job_description')
                description = str(desc_tag) if desc_tag else "Description not found."

                jobs.append({
                    'title': title,
                    'company': 'Mountain Air Cargo',
                    'location': location,
                    'url': job_url,
                    'description': description,
                })
                
            logger.info(f"[{self.site_key}] Successfully parsed {len(jobs)} potential new jobs.")
            return jobs
