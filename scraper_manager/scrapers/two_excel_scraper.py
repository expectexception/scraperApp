...
class TwoExcelScraper(BaseScraper):
    """Scraper for 2Excel"""

    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key="two_excel", db_manager=db_manager)
        self.site_config = config.get("sites", {}).get("two_excel", {})
        self.base_url = self.site_config.get("base_url", "https://2excel.talosats-careers.com")
        self.jobs_url = self.site_config.get("jobs_url", "https://2excel.talosats-careers.com/view-all-vacancies?what=&where=&iso=gb&radius=30&custom=-1-_-1-#vacancies-section-filters")
        self.company_name = "2Excel"
                ...
                for link in links:
                    ...
                    try:
                        ...
                        if "/job/" in href_lower or "/vacancy/" in href_lower or "/position/" in href_lower or "jobid=" in href_lower or "/view/" in href_lower or "/careers/" in href_lower or "/board/" in href_lower or "-jobs" in href_lower or "/offer/" in href_lower or "requisition" in href_lower:
                            ...
                            job_id = f"two_excel_{abs(hash(job_url)) % 10000000}"
