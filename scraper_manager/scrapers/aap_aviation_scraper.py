    ...
    async def fetch_jobs_from_listing(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    await page.goto(
                        self.jobs_url, wait_until="networkidle", timeout=30000
                    )
                except:
                    await page.goto(
                ...
                print(
                    f"  Starting scroll loop (Target: {self.max_jobs or 'Unlimited'} jobs)..."
                )
                seen_count = 0
                ...
                while True:
                    ...
                    if current_count > seen_count:
                        ...
                        print(
                            f"  No new jobs loaded (Attempt {no_new_content_count}/3)"
                        )
                ...
                if not job_elements:
                    ...
                    if job_links:
                        ...
                        for link in (
                            job_links[: self.max_jobs] if self.max_jobs else job_links
                        ):
                            try:
    ...
    async def _extract_description(self, browser, job):
        ...
        try:
            ...
            for p in paragraphs:
                ...
                if (
                    text.strip() and len(text.strip()) > 20
                    ...
                    if not job.get("location") or job.get("location") == "Unknown":
                        for pattern in location_patterns:
                            ...
                            if match:
                                ...
                                if (
                                    len(location.strip()) > 2
                                    and len(location.strip()) < 50
                                ):
                                    job["location"] = location.strip()
                                    break
                        ...
                        try:
                            ...
                            for item in items:
                                if isinstance(item, dict) and item.get("datePosted"):
                                    ...
                                    job["posted_date"] = (
                                        parsed if parsed else posted_candidate
                                    )
                                    break
                        except Exception:
                            continue
