...
class AerLingusScraper(BaseScraper):
    """
    Scraper for Aer Lingus (Workday).
    URL: https://aerlingus.wd3.myworkdayjobs.com/en-US/AerLingus
    """
    
    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key='aerlingus', db_manager=db_manager)
        self.base_url = "https://aerlingus.wd3.myworkdayjobs.com/en-US/AerLingus"
        self.company_name = "Aer Lingus"
            ...
            try:
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        real_title = title
                        h1 = detail_page.locator('h2, h1').first
                        ...
                        description = ""
                        for loc in ['[data-automation-id="jobPostingDescription"]', '.job-description', 'main']:
                        ...
                        location = "Ireland"
                        for loc in ['[data-automation-id="locations"]', '.location']:
                            elem = detail_page.locator(loc).first
                            if await elem.is_visible():
                                loc_text = await elem.inner_text()
                                if loc_text: location = loc_text.replace('Locations', '').strip()
                                break
                                
                        posted_date = await self.extract_posted_date_from_page(detail_page)
                        
                        job_id = f"aerlingus_{i+1}"
                        match = re.search(r'job/[^/]+/([^/]+)$', url)
                        if match and match.group(1):
                            job_id = f"aerlingus_{match.group(1)}"
