...
class AscentScraper(BaseScraper):
    """Scraper for Ascent Flight Training"""

    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key="ascent", db_manager=db_manager)
        self.site_config = config.get("sites", {}).get("ascent", {})
        self.base_url = self.site_config.get("base_url", "https://ascentflighttraining.com")
        self.jobs_url = self.site_config.get("jobs_url", "https://ascentflighttraining.com/careers/")
        self.company_name = "Ascent Flight Training"
                ...
                for link in links:
                    ...
                    try:
                        ...
                        if "/job/" in href_lower or "/vacancy/" in href_lower or "/position/" in href_lower or "jobid=" in href_lower or "/view/" in href_lower or "/careers/" in href_lower or "/board/" in href_lower or "-jobs" in href_lower or "/offer/" in href_lower or "requisition" in href_lower:
                            ...
                            job_id = f"ascent_{abs(hash(job_url)) % 10000000}"
