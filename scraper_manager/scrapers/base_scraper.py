...
class BaseScraper:
    ...
    def __init__(self, config: dict, site_key: str, db_manager=None):
        ...
        self.impersonate = random.choice(self.impersonate_list) if self.impersonate_list else 'chrome110'
        
        # Headless setting (can be overridden per-site in config.py)
        # Default to True, but check if explicitly set in scraper config
        self.headless = config.get('scrapers', {}).get(site_key, {}).get('headless', True)
    ...
    async def make_request(self, url: str, method: str = 'GET', **kwargs):
        ...
        if 'impersonate' not in kwargs:
            if self.impersonate_list:
                ...
                logger.debug(f"[{self.site_key}] Rotating impersonation: {impersonate_target}")
        ...
        async with curl_requests.AsyncSession() as s:
            try:
                ...
                logger.debug(f"[{self.site_key}] {method} request to {url} (Impersonating: {impo})")
    ...
    def parse_salary(self, salary_text: str) -> Dict:
        ...
        # Detect currency
        if '£' in text or 'gbp' in text: result['salary_currency'] = 'GBP'
        elif '€' in text or 'eur' in text: result['salary_currency'] = 'EUR'
        elif '₹' in text or 'inr' in text: result['salary_currency'] = 'INR'
        elif 'aed' in text: result['salary_currency'] = 'AED'
    ...
    def is_job_link(self, title: str, url: str) -> bool:
        ...
        path = u.replace('https://', '').replace('http://', '').split('/', 1)[-1] if '/' in u.replace('://', '___') else ''
        ...
        skip_words = [
            ...
            'contact', 'home', 'about', 'news', 'faq', 'help', 'login', 
