            ...
            try:
                ...
                for i, row in enumerate(rows):
                    ...
                    try:
                        ...
                        full_link = f"{self.domain}{relative_link}" if relative_link else self.base_url
                        ...
                        try:
                            ...
                            for loc in content_locators:
                                if await detail_page.locator(loc).first.is_visible():
                                    description = await detail_page.locator(loc).first.inner_html()
                                    break
                            
                            if not description:
                                description = await detail_page.content() # Fallback to full content if specific area not found
                                
                            job['description'] = description
                            
                            # Check for Apply button URL
                            apply_btn = detail_page.locator('text=Apply') # Simple text match
                            if await apply_btn.count() > 0:
                                ...
                                if apply_href:
                                     job['apply_url'] = f"{self.domain}{apply_href}" if apply_href.startswith('/') else apply_href
                            
                            await detail_page.close()
                            
                        except Exception as e:
                            logger.warning(f"[{self.site_key}] Failed to fetch details for {title}: {e}")
