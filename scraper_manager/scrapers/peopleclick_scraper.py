    ...
    async def fetch_jobs(self) -> List[Dict[str, Any]]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(
                    f"[{self.site_key}] Establishing session at {self.jobs_url}"
                )
                ...
                if search_btn:
                    logger.info(
                        f"[{self.site_key}] Clicking search button to initialize session..."
                    )
                ...
                if not api_response or api_response.get("status") == "fail":
                    logger.error(
                        f"[{self.site_key}] API Error: {api_response.get('errorMsg', 'Unknown error')}"
                    )
                    return []
                ...
                for raw_job in job_list:
                    ...
                    description = (
                        self.clean_html(description_html)
                        if description_html
                    ...
                    job_data = get_job_dict(
                        ...
                        posted_date=self.parse_posted_date(attr.get("JP_POSTEDON"))
                        if attr.get("JP_POSTEDON")
                        else None,
