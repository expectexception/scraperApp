...
class GlobalJetScraper(BaseScraper):
    ...
    async def fetch_jobs(self) -> list:
        logger.info(
            f"[{self.site_key}] Navigating to {self.jobs_url} (Headless={self.headless})..."
        )
        jobs = []
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(
                    f"[{self.site_key}] Found {len(vacancy_items)} vacancy items"
                )
                ...
                for idx, item in enumerate(vacancy_items):
                    ...
                    try:
                        ...
                        details_btn = await item.query_selector(
                            ".vacancies__link button"
                        )
                        if details_btn:
                            ...
                            try:
                                await page.wait_for_selector(
                                    modal_selector, timeout=5000
                                )
                                desc_elem = await page.query_selector(
                                    f"{modal_selector} .__content"
                                )
                                description = (
                                    (await desc_elem.inner_text()).strip()
                                    if desc_elem
                                    else ""
                                )
                                ...
                                close_btn = await page.query_selector(
                                    ".button-close-modal.js-close-modal"
                                )
                                if close_btn:
                                ...
                                logger.warning(
                                    f"[{self.site_key}] Could not open modal for {title}: {e}"
                                )
                                description = ""
                        else:
                            description = ""
                        ...
                        logger.warning(
                            f"[{self.site_key}] Error processing item {idx}: {e}"
                        )
