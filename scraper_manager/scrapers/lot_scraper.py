    ...
    async def fetch_jobs(self) -> list:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"[{self.site_key}] Error accessing site: {response.status_code}")
                return []
            ...
            for l in links:
                ...
                is_career_link = any(x in href.lower() for x in [
                    'pilot', 'cabin-crew', 'job', 'career', 'recruitment', 
                    'kariera', 'praca-w-samolocie', 'praca-w-biurze', 'operacje-lotnicze'
                ])
                ...
                if is_career_link:
                    if href not in seen:
                        ...
                        if not href.startswith('http'):
                            href = "https://www.lot.com" + href if href.startswith('/') else "https://www.lot.com/" + href
            ...
            for i, link_data in enumerate(matched_links):
                ...
                try:
                    job_resp = requests.get(link_data["url"], impersonate="chrome110", timeout=15)
                    job_soup = BeautifulSoup(job_resp.content, 'html.parser')
                    
                    real_title = link_data["title"]
                    h1 = job_soup.find('h1', class_='hero-banner__title') or job_soup.find('h1')
                    if h1 and h1.text.strip():
                        real_title = h1.text.strip()
                        
                    description = ""
                    desc_elem = job_soup.find('div', class_=lambda c: c and 'content' in c.lower())
                    if desc_elem:
                        description = str(desc_elem)
                    else:
                        description = str(job_soup.body) if job_soup.body else "Details in URL."
                    ...
                    logger.error(f"[{self.site_key}] Error parsing {link_data['url']}: {e}")
