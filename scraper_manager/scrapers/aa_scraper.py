    ...
    async def fetch_jobs(self) -> List[Dict[str, Any]]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if "Access Denied" in title or "Attention Required" in title or not await page.query_selector('a[href*="/job/"]'):
                    logger.warning(f"[{self.site_key}] Browser blocked or results not found. Trying to use make_request fallback...")
                ...
                if not jobs:
                    ...
                    job_selectors = [
                        'tr.data-row', 'tr.job-row', 'li[data-testid="jobCard"]', 
                        '.job-listing-item', 'div[class*="job-card"]'
                    ]
                    ...
                    for sel in job_selectors:
                        ...
                        if target_rows:
                            logger.info(f"[{self.site_key}] Found {len(target_rows)} jobs using selector: {sel}")
                            break
                    ...
                    if target_rows:
                        ...
                        for row in target_rows:
                            ...
                            try:
                                ...
                                loc_el = await row.query_selector('.jobLocation, .location, [class*="location"]')
                                
                                if link_el:
                                    title_text = await link_el.inner_text()
                                    href = await link_el.get_attribute('href')
                                    location_text = await loc_el.inner_text() if loc_el else "Unknown"
                                    
                                    jobs.append({
                                        'title': title_text.strip(),
                                        'url': "https://jobs.aa.com" + href if href.startswith('/') else href,
                        ...
                        links = await page.query_selector_all('a[href*="/job/"]:not([href*="facebook"]):not([href*="twitter"])')
                        if links:
                            logger.info(f"[{self.site_key}] Found {len(links)} job links via fallback")
                            for link in links:
                                if len(jobs) >= self.max_jobs: break
                    ...
                    try:
                        ...
                        for sel in apply_selectors:
                             ...
                             if apply_btn:
                                  ...
                                  if href:
                                       job['apply_url'] = href if href.startswith('http') else f"https://jobs.aa.com{href}"
                                       break
                        ...
                        logger.warning(f"[{self.site_key}] Failed to fetch details for {job.get('title')}: {e}")
                        try:
                             await detail_page.close()
                        except:
                             pass
