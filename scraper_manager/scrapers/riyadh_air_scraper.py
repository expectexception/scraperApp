    ...
    async def fetch_jobs(self) -> list:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                await page.goto(self.search_url, wait_until='domcontentloaded', timeout=90000)
                ...
                try:
                    ...
                    logger.warning(f"[{self.site_key}] iCIMS iframe not found, attempting to scrape main page")
                    iframe = page

                # Wait for job rows within the iframe
                try:
                    await iframe.locator('.iCIMS_JobsTable .row').first.wait_for(timeout=20000)
                    logger.info(f"[{self.site_key}] Found job rows in iframe")
                except Exception:
                    logger.warning(f"[{self.site_key}] Timeout waiting for job rows in iframe.")
                ...
                while has_next and page_num <= 5: # Limit to 5 pages
                    ...
                    logger.info(f"[{self.site_key}] Found {len(job_elements)} job rows on page {page_num}")
                    ...
                    for element in job_elements:
                        ...
                        if await title_anchor.count() > 0:
                            ...
                            loc_elem = element.locator('.header.left span:nth-of-type(2)')
                            location = (await loc_elem.inner_text()).strip() if await loc_elem.count() > 0 else "Riyadh"
                    ...
                    logger.info(f"[{self.site_key}] {len(matched_batch)}/{len(batch_jobs)} jobs passed pre-filter on page {page_num}")
                    ...
                    for j_initial in matched_batch:
                        ...
                        try:
                            ...
                            desc_elem = await detail_page.query_selector('.iCIMS_JobListing, .iCIMS_InfoMsg_Job')
                            if desc_elem:
