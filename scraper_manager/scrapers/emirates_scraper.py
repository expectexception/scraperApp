...
class EmiratesScraper(BaseScraper):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"[{self.site_key}] Initial navigation timeout/error: {e}"
                    )
                ...
                try:
                    ...
                    if accept_btn:
                        ...
                        await page.wait_for_selector(
                            "#onetrust-banner-sdk", state="hidden", timeout=5000
                        )
                        logger.info(f"[{self.site_key}] Accepted cookies")
                except Exception:
                    logger.info(
                        f"[{self.site_key}] No cookie banner found or already accepted"
                    )
                ...
                logger.info(
                    f"[{self.site_key}] Found {len(job_elements)} jobs on the list"
                )
                ...
                for el in job_elements:
                    ...
                    title_el = await el.query_selector(
                        "h4.job-title"
                    ) or await el.query_selector(".job-card__title")
                ...
                for job_meta in matched_initial:
                    ...
                    try:
                        job_data = await self._scrape_job_detail(
                            page, detail_url, job_meta
                        )
                        if job_data:
                        ...
                        logger.error(
                            f"[{self.site_key}] Error processing job {job_id}: {e}"
                        )
    ...
    async def _scrape_job_detail(
        self, page: Page, url: str, meta: Dict
    ) -> Optional[Dict]:
        """
