...
class HyperionScraper(BaseScraper):
    """
    Scraper for Hyperion Aero
    URL: https://hyperion.aero/job-openings/
    """
    def __init__(self, config: Dict[str, Any], db_manager=None):
        super().__init__(config, site_key="hyperion", db_manager=db_manager)
        self.base_url = "https://hyperion.aero/job-openings/"
        self.company_name = "Hyperion Aviation"
            ...
            try:
                ...
                for link in links:
                    ...
                    try:
                        ...
                        if "hyperion.aero" in href_lower and any(kw in href_lower for kw in ["job-openings/", "vacancy", "career", "job"]):
                            if text.lower() in ["read more", "apply", "apply now", "view details", "job openings"]:
                                continue
                            ...
                            job_id = f"hyperion_{abs(hash(job_url)) % 10000000}"
                            jobs.append({
                                "job_id": job_id,
                                "title": text,
