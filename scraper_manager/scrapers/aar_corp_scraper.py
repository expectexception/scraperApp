            ...
            try:
                ...
                try:
                    ...
                    if await clear_btn.is_visible():
                        logger.info(
                            f"[{self.site_key}] Clicking clearButton to reset filters..."
                        )
                        ...
                        if await view_all.is_visible():
                            logger.info(
                                f"[{self.site_key}] Clicking viewAllJobsLink..."
                            )
                    ...
                    if await switch_view.is_visible():
                        ...
                        if "Single-line" in view_text:
                            logger.info(
                                f"[{self.site_key}] Switching to Single-line view (found 'Single-line' in link text)..."
                            )
                            await switch_view.click()
                            await page.wait_for_timeout(5000)
                        else:
                            logger.info(
                                f"[{self.site_key}] Already in Single-line view (link text is '{view_text}')."
                            )
                except Exception as e:
                    logger.debug(
                        f"[{self.site_key}] No clear/view-all/switch-view button found or clickable: {e}"
                    )
                ...
                while True:
                    ...
                    if not job_rows:
                        logger.warning(
                            f"[{self.site_key}] Still no job rows found on page {page_count}"
                        )
                    ...
                    for job_data in page_jobs_data:
                        ...
                        try:
                            ...
                            taleo_desc = await detail_page.query_selector(
                            ...
                            job_id = (
                                f"aar_{job_id_match.group(1)}"
                                if job_id_match
                                else f"aar_{hash(url)}"
                            )

                            posted_date = (
                                self.parse_posted_date(job_data["dateText"])
                                if job_data.get("dateText")
                                else None
                            )
