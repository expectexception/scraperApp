        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    ...
                    try: 
                        cookie_btn = page.locator('#gdpr-cookie-accept, .cookie-accept-btn')
                        if await cookie_btn.count():
                            await cookie_btn.first.click()
                    except: pass

                    # Close bot/chat if it exists
                    try: await page.locator('.phenom-bot-close').click(timeout=2000)
                    except: pass
                ...
                while True:
                    ...
                    for item in job_items:
                        ...
                        try:
                            ...
                            loc_el = item.locator('[data-ph-at-id="job-location-text"]') or item.locator('.job-location')
                            location = (await loc_el.text_content() if await loc_el.count() else "United States").strip().replace('\n', ' ')
                            ...
                            if not link.startswith('http'):
                                link = f"https://careers.united.com{link}" if link.startswith('/') else f"https://careers.united.com/us/en/{link}"
                            ...
                            logger.debug(f"[{self.site_key}] Error parsing job item: {e}")
                            continue
        ...
        async with async_playwright() as p:
            ...
            for job in jobs:
                ...
                try:
                    ...
                    desc_el = page.locator('.job-description, .job-details-brief, .content')
                    job['description'] = await desc_el.first.inner_html() if await desc_el.count() else await page.content()
                    
                    # Look for explicit Apply button
                    apply_btn = page.locator('a.primary-button, a[data-ph-at-id="apply-link"]')
                    if await apply_btn.count():
                        href = await apply_btn.first.get_attribute('href')
                        if href:
                            job['apply_url'] = href if href.startswith('http') else f"https://careers.united.com{href}"
                except Exception as e:
