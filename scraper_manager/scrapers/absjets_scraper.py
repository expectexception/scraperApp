        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for link in links:
                    ...
                    if href:
                        full_link = (
                            f"{self.domain}{href}" if href.startswith("/") else href
                        )
                ...
                for i, url in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        desc_loc = detail_page.locator(
                            ".cp-detail__content, .teamio-detail-content, .job-description, .detail-content, .content, main"
                        )
                        ...
                        loc_element = detail_page.locator(
                            ".cp-info__item--location .cp-info__item-link, .cp-info__item--location"
                        ).first
                        if await loc_element.is_visible():
                            location = await loc_element.inner_text()
                        else:
                            m_loc = re.search(
                                r"(Praha|Bratislava|Brno|Ostrava).*",
                                content,
                                re.IGNORECASE,
                            )
                            if m_loc:
