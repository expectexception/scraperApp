...
class AviationIndeedScraper(BaseScraper):
    ...
    async def run(self):
        ...
        print(
            f"Fetching jobs from {self.site_config.get('name', 'Aviation Indeed')}..."
        )
        ...
        if not jobs:
            print(
                "❌ No jobs found with current layout – falling back to legacy iframe check"
            )
    ...
    async def fetch_jobs_current_layout(self):
        ...
        try:
            ...
            if not response or response.status_code != 200:
                print(
                    f"❌ make_request failed: {response.status_code if response else 'No response'}"
                )
                return []
            ...
            if next_data_script:
                try:
                    ...
                    if not jobs_list:
                        ...
                        queries = page_props.get("dehydratedState", {}).get(
                            "queries", []
                        )
                        for q in queries:
                            ...
                            if (
                                ...
                                and (
                                    len(res) > 0
                                    and ("title" in res[0] or "job_title" in res[0])
                                )
                            ):
                                jobs_list = res
                                break
                    ...
                    if jobs_list:
                        ...
                        for idx, job in enumerate(jobs_list):
                            ...
                            job_data_id = str(
                                job.get("id", job.get("job_id", f"aviation_{idx}"))
                            )
                            location = job.get(
                                "location", job.get("job_location", "Remote/Global")
                            )
                            posted_date = job.get(
