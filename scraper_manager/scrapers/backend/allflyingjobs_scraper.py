...
class AllFlyingJobsScraper(BaseScraper):
    ...
    async def fetch_jobs(self) -> List[Dict[str, Any]]:
        ...
        if not search_queries:
            print(
                f"No search queries configured for {self.site_key}. Please add 'search_queries' list to config."
            )
            return []
    ...
    async def _scrape_query(self, page: Page, context, query: str):
        ...
        while len(self.jobs) < max_jobs:
            print(
                f"Processing page {page_num} for query '{query}' (Total: {len(self.jobs)}/{max_jobs})..."
            )
            ...
            for card in cards:
                ...
                try:
                    ...
                    company_el = await card.query_selector(
                        ".field-name-field-recruiter-name .field-item"
                    )
                    ...
                    if not location_el:
                        location_el = await card.query_selector(
                            ".field-name-field-base .field-item"
                        )
                    location = (
                    ...
                    if url:
                        ...
                        try:
                            ...
                            job[
                                "description"
                            ] = await self.extract_description_from_page(detail_page)
                        except Exception as e:
    ...
    async def run(self):
        print("--- Starting AllFlyingJobs Scraper ---")
