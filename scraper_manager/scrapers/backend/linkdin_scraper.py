...
class LinkedInScraper(BaseScraper):
    ...
    async def scrape_linkedin_jobs(
        self, page, post_name: str, location_name: str, max_jobs: int
    ) -> List[Dict]:
        ...
        try:
            ...
            search_url_with_params = (
                f"{self.search_url}?keywords={keywords}&location={location}"
            )
            ...
            try:
                await page.wait_for_selector(
                    "ul.jobs-search__results-list", timeout=30000
                )
        ...
        job_cards = await page.locator(
            "ul.jobs-search__results-list div.base-search-card"
        ).all()
        ...
        if not job_cards or len(job_cards) == 0:
            print(
                f"⚠️ No job cards found for '{post_name}' in '{location_name}'. Skipping to next search..."
            )
            return []

        print("Scraping ALL job cards and applying title filtering...")
        ...
        for i in range(len(job_cards)):
            ...
            if i in failed_card_indices:
                print(
                    f"⊘ Skipping job card {i + 1} (failed in previous search - marked to retry)"
                )
                continue

            # Refetch job cards fresh each time to avoid stale references
            current_job_cards = await page.locator(
                "ul.jobs-search__results-list div.base-search-card"
            ).all()

            if not current_job_cards or len(current_job_cards) == 0:
                print(
                    "⚠️ Job cards no longer available. All cards processed. Moving to next search..."
                )
                break
            ...
            try:
                ...
                company = await job_card.locator(
                    "h4.base-search-card__subtitle"
                ).inner_text(timeout=5000)
                company = company.strip()

                location = await job_card.locator(
            ...
            job_data["job_id"] = (
                f"linkedin_{post_name.replace(' ', '_')}_{location_name.replace(' ', '_')}_{i + 1}"
            )
            ...
            try:
                ...
                if (
                    ...
                    or len(job_data.get("description") or "") < 80
                ):
                    try:
                        fallback_desc = await self.extract_description_from_page(page)
                        if fallback_desc and len(fallback_desc) > len(
                ...
                additional_details = {}
                try:
                    insight_items = await page.locator(
                ...
                job_data["salary_currency"] = additional_details.get(
                    "salary_currency", "USD"
                )
                ...
                print("   Scrolling back to job card list...")
            ...
            try:
                ...
                if await page.locator("div.description__text").count() > 0:
                    print("   Going back to job card list...")
    ...
    async def run(self):
        ...
        posts = self.site_config.get(
            "search_post", self.site_config.get("default_post", self.default_posts)
        )
        if isinstance(posts, str):
            posts = [posts]

        locations = self.site_config.get(
            "search_location",
            self.site_config.get("default_location", self.default_locations),
        )
        ...
        if self.max_jobs:
            ...
            max_jobs_total = self.site_config.get(
                "max_jobs_total", max_jobs_per_search * len(posts) * len(locations)
            )
        ...
        print(
            f"Max jobs per search: {max_jobs_per_search}, Total limit: {max_jobs_total}"
        )
            ...
            try:
                ...
                for post_name in posts:
                    ...
                    for location_name in locations:
                        ...
                        print(f"\n{'=' * 60}")
                        print(
                            f"Search {search_count}/{total_searches}: '{post_name}' in '{location_name}'"
                        )
                        print(f"{'=' * 60}")
                        ...
                        try:
                            jobs = await self.scrape_linkedin_jobs(
                                page, post_name, location_name, jobs_per_search
                            )
                            ...
                            if jobs:
                                ...
                                self.collected_jobs = (
                                    all_jobs.copy()
                                )  # Track collected jobs
                                print(
                                    f"✓ Found {len(jobs)} jobs for '{post_name}' in '{location_name}'"
                                )
                            else:
                                print(
                                    f"⚠️ No jobs found for '{post_name}' in '{location_name}'"
                                )

                        except Exception as e:
                            print(
                                f"✗ Error scraping '{post_name}' in '{location_name}': {e}"
                            )
                            continue
                        ...
                        if search_count < total_searches:
                            await self.random_delay(
                                5, 10
                            )  # 5-10 second delay between searches
        ...
        if self.use_filter and self.filter_manager:
            ...
            matched_jobs, rejected_jobs, filter_stats = self.apply_title_filter(
                all_jobs
            )
