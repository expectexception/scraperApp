                ...
                while job_count < self.max_jobs:
                    ...
                    try:
                        await page.wait_for_selector(
                            "div.jcl-job-teaser", timeout=10000
                        )
                    except Exception:
                    ...
                    for card in cards:
                        try:
                            ...
                            location_el = await card.query_selector(
                                ".jcl-job-teaser-location span.popoverlist-no-list-item"
                            )
                            location = (
                    ...
                    print(
                        f"Applying pre-filter to {len(page_jobs)} jobs on page {page_num}..."
                    )
    ...
    async def extract_description_from_page(self, page: Page) -> str:
        ...
        try:
            ...
            desc_el = await page.query_selector(
                ".job-description, .job-details, article, main"
            )
            if desc_el:
