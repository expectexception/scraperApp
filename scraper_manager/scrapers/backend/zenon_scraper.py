    ...
    async def fetch_jobs(self) -> List[Dict[str, Any]]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(
                    f"[{self.site_key}] Found {len(job_elements)} job title headers"
                )
                ...
                for el in job_elements:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[{self.site_key}] Failed to extract location for {title}: {e}"
                        )
                ...
                logger.info(
                    f"[{self.site_key}] Fetching details for {len(jobs)} jobs..."
                )
                ...
                for i, job in enumerate(jobs):
                    try:
                        ...
                        logger.error(
                            f"[{self.site_key}] Error processing details for {job['title']}: {e}"
                        )
