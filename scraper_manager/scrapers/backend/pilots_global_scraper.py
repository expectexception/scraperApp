        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                while current_page <= max_pages and len(jobs) < self.max_jobs:
                    ...
                    logger.info(
                        f"[{self.site_key}] Scraping page {current_page}: {page_url}"
                    )
                    ...
                    try:
                        ...
                        if not job_elements:
                            logger.info(
                                f"[{self.site_key}] No more jobs found on page {current_page}"
                            )
                            break

                        logger.info(
                            f"[{self.site_key}] Found {len(job_elements)} job cards on page {current_page}"
                        )
                        ...
                        for element in job_elements:
                            ...
                            try:
                                ...
                                await element.query_selector_all("span")
                        ...
                        logger.error(
                            f"[{self.site_key}] Error scraping page {current_page}: {e}"
                        )
                        break
    ...
    async def _fetch_descriptions(self, page: Page, jobs: List[Dict]):
        ...
        for i, job in enumerate(jobs):
            ...
            try:
                ...
                await page.goto(
                    job["source_url"], wait_until="networkidle", timeout=30000
                )
                ...
                logger.warning(
                    f"[{self.site_key}] Failed to fetch description for {job['title']}: {e}"
                )
