...
class JSFirmScraper(BaseScraper):
    ...
    async def _scrape_location(self, page: Page, context, location: str):
        ...
        while len(self.jobs) < self.max_jobs:
            print(
                f"Processing page {page_num} for {location} (Total jobs: {len(self.jobs)}/{self.max_jobs})..."
            )
            ...
            for card in cards:
                try:
                    ...
                    date_posted = (
                        (await date_el.inner_text()).strip() if date_el else "Unknown"
                    )
            ...
            print(
                f"{len(matched_page_jobs)} jobs passed pre-filtering. Fetching details..."
            )
            ...
            if len(self.jobs) < self.max_jobs:
                ...
                if next_btn:
                    ...
                    if cards:
                        try:
                            title_el = await cards[0].query_selector(
                                "a.u:not(.company)"
                            )
                            if title_el:
                    ...
                    try:
                        ...
                        async def check_new_content():
                            ...
                            while (datetime.now() - start_time).seconds < 30:
                                try:
                                    ...
                                    new_title_el = await new_cards[0].query_selector(
                                        "a.u:not(.company)"
                                    )
                                    if new_title_el:
                        ...
                        if await check_new_content():
                            ...
                            print(
                                "Warning: Content did not appear to change after pagination click."
                            )
                            break
                    except Exception as e:
