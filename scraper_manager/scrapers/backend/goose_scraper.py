                ...
                for link in job_links:
                    try:
                        ...
                        if (
                            title
                            and len(title) > 10
                            and title not in ["Read More", "Apply Now", "Save Job"]
                        ):
                            job_id = (
                                href.split("/")[-1]
                                if "/" in href
                                else f"goose_{len(jobs)}"
                            )
    ...
    async def _extract_description(self, browser, job):
        ...
        try:
            ...
            if page_title and len(page_title) > 10:
                ...
                if (
                    title_clean
                    and title_clean != "Aviation Jobs - Airline Jobs - Pilot Jobs"
                ):
            ...
            if main_desc_elem:
                ...
                if len(paragraph_texts) < 2:
                    ...
                    lines = [
                        line.strip()
                        for line in full_container_text.split("\n")
                        if line.strip()
                    ]
                    ...
                    if not paragraph_texts:
                        ...
                        description_parts.extend(
                            [l for l in lines if len(l) > 30]
                        )  # Only substantial lines
