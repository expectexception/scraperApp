...
class FlygoshScraper(BaseScraper):
            ...
            async with async_playwright() as p:
                ...
                for idx, card in enumerate(job_cards):
                    ...
                    try:
                        ...
                        title_el = await card.query_selector('.fg-job-title a, .job-title a, h3 a, h4 a')
                        if not title_el:
                            title_el = await card.query_selector('a[href*="/job/"]:not(:empty)')
                        ...
                        if not title or not title.strip():
                            ...
                            title_div = await card.query_selector('.fg-job-title, .job-title, h3, h4')
                            if title_div:
                        ...
                        company_el = await card.query_selector('a[href*="/company-jobs/"]')
                        company = await company_el.inner_text() if company_el else self.company_name
    ...
    async def fetch_job_descriptions(self, jobs: List[Dict]) -> List[Dict]:
        ...
        async with async_playwright() as p:
            ...
            for i in range(0, len(jobs), self.batch_size):
                ...
                logger.info(f"Batch {i//self.batch_size + 1}/{(len(jobs)-1)//self.batch_size + 1} (jobs {i+1}-{min(i+self.batch_size, len(jobs))})")
    ...
    async def run(self):
        ...
        if self.use_filter and self.filter_manager:
            ...
            logger.info(f"✓ {len(matched_jobs)} jobs matched filter (will fetch descriptions)")
