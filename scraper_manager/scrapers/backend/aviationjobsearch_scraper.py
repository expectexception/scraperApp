...
class AviationJobSearchScraper(BaseScraper):
    ...
    def print_category_summary(self, jobs):
        ...
        # Print each category
        for category, cat_jobs in sorted(
    ...
    async def _extract_description(self, browser, job):
        ...
        try:
            ...
            if len(url_parts) > 4:
                ...
                url_parts[5] if len(url_parts) > 5 else ""
                        ...
                        try:
                            ...
                            for item in items:
                                if isinstance(item, dict):
                                    ...
                                    if "JobPosting" in str(item_type) or item.get(
                                        "datePosted"
                                    ):
                                        if item.get("datePosted"):
                                            posted_candidate = item.get("datePosted")
                                            parsed = self.parse_posted_date(
                                                posted_candidate
                                            )
                                            job["posted_date"] = (
            ...
            if not job["posted_date"]:
                try:
                    date_elem = await page.query_selector(
                        '[class*="date"], [class*="posted"], time, [class*="Date"]'
                    )
                    if date_elem:
            ...
            if main_desc_elem:
                ...
                for line in lines:
                    ...
                    if (
                        any(keyword in line.lower() for keyword in skip_keywords)
                        and len(line) < 30
                    ):
                        continue
