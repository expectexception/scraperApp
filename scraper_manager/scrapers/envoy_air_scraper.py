        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                while True:
                    ...
                    if iframe_el:
                        logger.info(
                            f"[{self.site_key}] Switching context to iCIMS iframe..."
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[{self.site_key}] Timeout waiting for jobs on page {page_count}"
                        )
                        break
                    ...
                    if not job_rows:
                        logger.warning(
                            f"[{self.site_key}] No job rows found on page {page_count}"
                        )
                        break
                    ...
                    for row in job_rows:
                        try:
                            ...
                            id_el = row.locator(
                                'span:has-text("ID") + span, .iCIMS_JobHeaderGroup dl:has(dt:has-text("ID")) dd'
                            ).first
                    ...
                    for job_data in page_jobs_data:
                        ...
                        try:
                            ...
                            logger.info(
                                f"[{self.site_key}] Fetching details for: {detail_url}"
                            )
                            ...
                            try:
                                await detail_page.wait_for_selector(
                                    ".iCIMS_JobDescription, .iCIMS_JobContents, .iCIMS_InfoTable_JobDescription, .iCIMS_InfoTable",
                                    timeout=10000,
                                )
                            except:
                                logger.debug(
                                    f"[{self.site_key}] Timeout waiting for description selector on detail page."
                                )

                            await detail_page.wait_for_timeout(2000)

                            # iCIMS specific description container
                            icims_desc = await detail_page.query_selector(
                                ".iCIMS_JobDescription, .iCIMS_JobContents, .iCIMS_InfoTable_JobDescription, .iCIMS_InfoTable"
                            )
                            if icims_desc:
                            ...
                            if not job_id:
                                ...
                                id_el = await detail_page.query_selector(
                                    '.iCIMS_JobHeaderTable dl:has(dt:has-text("ID")) dd'
                                )
                                if id_el:
                    ...
                    try:
                        next_btn = frame.locator(
                            'a.glyph[title*="Next page"], a:has(span.sr-only:has-text("Next page"))'
                        ).first
