        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                async def route_handler(route):
                    if "apply-custom-filters" in route.request.url and "$top=10" in route.request.url:
                        new_url = route.request.url.replace("$top=10", "$top=500")
                ...
                async def handle_response(response):
                    if (
                        "apply-custom-filters" in response.url
                ...
                try:
                    cookie_btn = await page.query_selector(
                        "button:has-text('Accept'), .optanon-allow-all-button"
                    )
                    if cookie_btn:
                ...
                try:
                    await page.wait_for_selector(
                        ".job-title-link, mat-expansion-panel", timeout=20000
                    )
                except:
                ...
                if api_response_data:
                    logger.info(
                        f"[{self.site_key}] Captured API data for {len(api_response_data)} responses."
                    )
                        ...
                        for req in requisitions:
                            ...
                            if locations:
                                ...
                                if "nameCode" in first_loc:
                                    location = first_loc["nameCode"].get(
                                        "longName", "Unknown"
                                    )
                    ...
                    logger.warning(
                        f"[{self.site_key}] No API data captured, falling back to DOM parsing..."
                    )
