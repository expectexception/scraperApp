        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        await page.wait_for_selector('.results-list__item', timeout=15000)
                    except:
                        logger.warning(f"[{self.site_key}] Timeout waiting for .results-list__item on page {page_count}")
                        break
                    ...
                    if not job_items:
                        logger.warning(f"[{self.site_key}] No job items found on page {page_count}")
                        break

                    logger.info(f"[{self.site_key}] Found {len(job_items)} job items on page {page_count}")
                    
                    page_jobs_data = []
                    for item in job_items:
                        try:
                            # Link and Title
                            link_el = await item.query_selector('a.results-list__item-title--link')
                            if not link_el:
                                continue
                    ...
                    for job_data in page_jobs_data:
                        ...
                        try:
                            ...
                            phenom_desc = await detail_page.query_selector('.job-description, .job-details-brief')
                            if phenom_desc:
