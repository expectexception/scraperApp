    ...
    async def fetch_jobs(self) -> list:
        ...
        try:
            async with async_playwright() as p:
                ...
                logger.info(
                    f"[{self.site_key}] Found {len(job_headers)} potential job rows"
                )
                ...
                for i, header in enumerate(job_headers):
                    ...
                    link_elem = header.locator(
                        'xpath=following-sibling::div[contains(@class, "td3")][1]//a[contains(@class, "botonAvisos")]'
                    ).first
                    if await link_elem.is_visible():
                        href = await link_elem.get_attribute("href")
                        full_url = (
                            f"https://empleo.aena.es/empleo/{href}"
                            if "http" not in href
                            else href
                        )
