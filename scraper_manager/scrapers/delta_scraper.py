            ...
            try:
                ...
                while len(jobs) < max_jobs_to_reach and offset < 2000:
                    ...
                    if "No jobs found" in content or "1-0 of" in content:
                        logger.info(f"[{self.site_key}] Reached end of results at offset {offset}")
                        break
                    ...
                    try:
                        ...
                        logger.info(f"[{self.site_key}] No job items found at offset {offset}")
                        break
                    ...
                    logger.info(f"[{self.site_key}] Found {len(items)} job items at offset {offset}")
                    
                    for item in items:
                        if len(jobs) >= max_jobs_to_reach:
                            break
                            
                        title_el = await item.query_selector(".list__item__text__title a")
                        if not title_el:
                            continue
                        ...
                        job_url = href if href.startswith('http') else f"https://delta.avature.net{href}"
                        ...
                        location = "Unknown"
                        loc_el = await item.query_selector(".list__item__text__subtitle span:first-child")
                        if loc_el:
                        ...
                        logger.info(f"[{self.site_key}] Successfully extracted: {title}")
    ...
    async def fetch_job_description_api(self, job_id: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                content = soup.select_one('.description-ajax, .article__content, [itemprop="description"]')
                if content:
                    return str(content)
