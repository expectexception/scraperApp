...
class OlympicAirScraper(BaseScraper):
    """
    Scraper for Olympic Air.
    They are a subsidiary of Aegean Airlines and list their jobs on jobs.aegeanair.com.
    """
    ...
    async def fetch_jobs(self) -> list:
        logger.info(f"[{self.site_key}] Navigating to Aegean careers portal proxy...")
        jobs = []
            ...
            try:
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        for loc in ['.jobdescription', '.job-description', 'main']:
                        ...
                        job_id = f"olympicair_{i+1}"
                        match = re.search(r'job/[^/]+/[^/]+(?:/(\d+))?', url)
                        if match and match.group(1):
                            job_id = f"olympicair_{match.group(1)}"
