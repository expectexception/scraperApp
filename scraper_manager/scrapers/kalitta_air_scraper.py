...
class KalittaAirScraper(BaseScraper):
    ...
    async def fetch_jobs(self) -> list:
        ...
        async with async_playwright() as p:
            ...
            try:
                # 1. Establish session
                logger.info(f"[{self.site_key}] Establishing session at {self.base_url}...")
                ...
                while True:
                    ...
                    logger.info(f"[{self.site_key}] Found {len(requisitions)} jobs in current API chunk.")
                    ...
                    for req in requisitions:
                        ...
                        try:
                            ...
                            logger.info(f"[{self.site_key}] Fetching detail API for: {item_id}")
                            ...
                            description_html = detail_json.get('requisitionDescription', '')
                            ...
                            logger.error(f"[{self.site_key}] Error fetching detail for {item_id}: {e}")
                            continue
