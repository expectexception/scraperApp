                    ...
                    if query:
                        try:
                            search_input = await page.query_selector(
                                'input[name="q"], input[type="search"], input[aria-label*="Search"]'
                            )
                            if search_input:
                            ...
                            logger.warning(
                                f"[{self.site_key}] Search interaction failed: {e}"
                            )
                    ...
                    while len(jobs) < self.max_jobs:
                        ...
                        try:
                            await page.wait_for_selector(
                                "tr.data-row, tr.job-row, .job-card, .job-listing-item",
                                timeout=10000,
                            )
                        except:
                            logger.debug(
                                f"[{self.site_key}] Timeout waiting for job rows"
                            )

                        # Extract jobs on current page using broader selectors
                        job_rows = await page.query_selector_all(
                            'tr.data-row, tr.job-row, tr[id*="job-result"], .job-card, .job-listing-item'
                        )

                        if not job_rows:
                            # Fallback to all job links
                            job_links = await page.query_selector_all(
                                'a.jobTitle-link, a[href*="/job/"]:not([href*="facebook"]):not([href*="twitter"])'
                            )
                            rows_data = []
                            unique_urls = set()
                            for link in job_links:
                                ...
                                if href and title.strip() and "/job/" in href:
                                    f_url = (
                                        "https://careers.wizzair.com" + href
                                        if href.startswith("/")
                                        else href
                                    )
                            ...
                            for row in job_rows:
                                try:
                                    ...
                                    loc_el = await row.query_selector(
                                        '.jobLocation, .location, span[class*="location"]'
                                    )
                        ...
                        logger.info(
                            f"[{self.site_key}] Found {len(rows_data)} jobs on page {current_page_num}"
                        )

                        if not rows_data:
                            logger.info(
                                f"[{self.site_key}] No jobs found on page {current_page_num}, stopping."
                            )
                            break
                        ...
                        for item in rows_data:
                            ...
                            try:
                                ...
                                page_title_el = await detail_page.query_selector(
                                    "h1.jobTitle, h1#job-title, .jobTitle, h1"
                                )
                                if page_title_el:
                                    ...
                                    if (
                                        ...
                                        and "APPLY NOW" not in page_title.upper()
                                    ):
                                        title = page_title.strip()
                                    else:
                                        # Use OG title
                                        og_title_el = await detail_page.query_selector(
                                            'meta[property="og:title"]'
                                        )
                                        if og_title_el:
                                            og_title = await og_title_el.get_attribute(
                                                "content"
                                            )
                                            if og_title:
                                ...
                                if not location or location == "Unknown":
                                    loc_el = await detail_page.query_selector(
                                        ".jobLocation, .facility, .location, span.custom-field"
                                    )
                                    if loc_el:
                                        ...
                                        try:
                                            ...
                                            if script_el:
                                                js_data = await detail_page.evaluate(
                                                    "el => el.innerText", script_el
                                                )
                                                data = json.loads(js_data)
                                                if (
                                                    isinstance(data, dict)
                                                    and "jobLocation" in data
                                                ):
                                                    loc_data = data["jobLocation"]
                                                    if (
                                                        isinstance(loc_data, dict)
                                                        and "address" in loc_data
                                                    ):
                                ...
                                apply_btn = await detail_page.query_selector(
                                    'a[class*="apply"], a:has-text("Apply")'
                                )
                                if apply_btn:
                                ...
                                logger.error(
                                    f"[{self.site_key}] detail error {url}: {e}"
                                )
                        ...
                        if next_el:
                            logger.info(
                                f"[{self.site_key}] Navigating to page {next_page_num}..."
                            )
                            await next_el.click()
                            ...
                            logger.info(
                                f"[{self.site_key}] No more pages found after page {current_page_num}."
                            )
                            break
