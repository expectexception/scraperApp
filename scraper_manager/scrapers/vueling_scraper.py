...
class VuelingScraper(BaseScraper):
    """
    Scraper for Vueling.
    URL: https://careers.vueling.com/
    Uses Eightfold/Workday ATS structure.
    """
    
    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key='vueling', db_manager=db_manager)
        self.base_url = "https://careers.vueling.com/jobs"
        self.company_name = "Vueling"
            ...
            try:
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        for loc in ['.jobdescription', '.job-description', 'main', '.description', '#job-body']:
                        ...
                        location = "Spain"
                        for loc in ['.jobGeoLocation', '.location', '.job-details-location']:
                            elem = detail_page.locator(loc).first
                            if await elem.is_visible():
                        ...
                        job_id = f"vueling_{i+1}"
                        match = re.search(r'jobs/([^/]+)-', url)
                        if match and match.group(1):
                            job_id = f"vueling_{match.group(1)}"
