            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        logger.warning(f"[{self.site_key}] Timeout waiting for tr.data-row at startrow={start_row}")
                        break

                    job_rows = await page.query_selector_all('tr.data-row')
                    if not job_rows:
                        logger.info(f"[{self.site_key}] No more jobs found at startrow={start_row}")
                        break
                    ...
                    for job_data in page_jobs_data:
                        ...
                        try:
                            ...
                            desc_el = await detail_page.query_selector('.jobdescription, .job-description, .content')
                            description = ""
                            if desc_el:
