    ...
    async def fetch_jobs(self) -> List[Dict]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    await page.wait_for_selector('.current-openings-item', timeout=30000)
                ...
                processed_urls = set() # We'll use title + location as a pseudo-URL if we can't find a real one
                
                # Find all job items
                job_items = await page.query_selector_all('.current-openings-item')
                logger.info(f"[{self.site_key}] Found {len(job_items)} job items on the initial page.")
                ...
                for i in range(len(job_items)):
                    ...
                    current_items = await page.query_selector_all('.current-openings-item')
                    ...
                    try:
                        await page.wait_for_selector('.job-description-details', timeout=15000)
                        ...
                        logger.warning(f"[{self.site_key}] Failed to load details for {title}: {e}")
                    
                    # Go back to list
                    back_btn = await page.query_selector('#recruitment_jobDescription_back')
                    if back_btn:
                        await back_btn.click()
                        await page.wait_for_selector('.current-openings-item', timeout=10000)
                    else:
                        logger.warning(f"[{self.site_key}] Back button not found, attempting reload...")
