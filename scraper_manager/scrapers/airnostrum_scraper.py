...
class AirNostrumScraper(BaseScraper):
    """
    Scraper for Air Nostrum.
    URL: https://jobs.airnostrum.es/?locale=en_US
    Uses SuccessFactors ATS structure.
    """
    
    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key='airnostrum', db_manager=db_manager)
        self.base_url = "https://jobs.airnostrum.es/?locale=en_US"
        self.company_name = "Air Nostrum"
    ...
    async def fetch_jobs(self) -> list:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    cookie_btn = page.locator('text=Accept, text=Aceptar').first
                ...
                links = await page.evaluate('''() => {
                    return Array.from(document.querySelectorAll('a'))
                        ...
                        .filter(a => a.t && a.t.length > 5 && (a.h.includes('/job/') || a.h.includes('/go/')))
                }''')
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        for loc in ['.jobdescription', '.job-description', 'main', '.job', '.joblayout']:
                        ...
                        job_id = f"airnostrum_{i+1}"
                        match = re.search(r'go/[^/]+/(\d+)/', url)
                        if match and match.group(1):
                            job_id = f"airnostrum_{match.group(1)}"
