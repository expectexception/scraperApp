...
class TAPScraper(BaseScraper):
    """
    Scraper for TAP Air Portugal.
    URL: https://careers.flytap.com/
    Uses SuccessFactors/similar ATS structure.
    """
    
    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key='tap', db_manager=db_manager)
        self.base_url = "https://careers.flytap.com/"
        self.company_name = "TAP Air Portugal"
            ...
            try:
                ...
                links = await page.evaluate('''() => {
                    return Array.from(document.querySelectorAll('a'))
                        ...
                        .filter(a => a.t && a.t.length > 5 && (a.h.includes('/job/') || a.h.includes('jobad')))
                }''')
                ...
                for i, (url, title) in enumerate(job_urls):
                    ...
                    try:
                        ...
                        description = ""
                        for loc in ['.jobdescription', '.job-description', 'main', '.content', '#job-details']:
