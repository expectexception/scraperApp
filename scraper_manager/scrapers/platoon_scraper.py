...
class PlatoonScraper(BaseScraper):
    """
    Scraper for Platoon Aviation
    URL: https://platoon-aviation.jobs.personio.de/
    """

    def __init__(self, config, db_manager=None):
        super().__init__(config, site_key="platoon", db_manager=db_manager)
        self.base_url = "https://platoon-aviation.jobs.personio.de/"
        self.company_name = "Platoon Aviation"

    async def fetch_jobs(self) -> list:
        """
        Scrape jobs from Platoon Aviation Personio XML feed
        """
        jobs = []
        ...
        try:
            ...
            response_obj = await self.make_request(method="GET", url=xml_url)
            ...
            for i, pos_xml in enumerate(positions):
                ...
                title_match = re.search(
                    r"<name>(?:<!\[CDATA\[(.*?)\]\]>|(.*?))</name>", pos_xml, re.DOTALL
                )
                if not title_match:
                    continue
                ...
                job_id_str = (
                    id_match.group(1).strip() if id_match else f"platoon_{i + 1}"
                )
                job_id = f"platoon_{job_id_str}"

                # Extract description (handle both CDATA and plain text)
                desc = ""
                desc_match = re.search(
                    r"<jobDescriptions>(.*?)</jobDescriptions>", pos_xml, re.DOTALL
                )
                if desc_match:
                    ...
                    content = re.sub(
                        r"<value>(.*?)</value>", r"\1", content, flags=re.DOTALL
                    )
                    desc = content.strip()

                if not desc or len(desc) < 50:
                    desc = "Platoon Aviation career opportunities. Please visit the official career portal for more details."

                # Extract location (handle both CDATA and plain text)
                loc_match = re.search(
                    r"<office>(?:<!\[CDATA\[(.*?)\]\]>|(.*?))</office>",
                    pos_xml,
                    re.DOTALL,
                )
                location = (
                    (loc_match.group(1) or loc_match.group(2) or "Germany").strip()
                    if loc_match
                    else "Germany"
                )

                # Extract URLs (we need to construct the URL for Personio jobs)
                url = f"https://platoon-aviation.jobs.personio.de/job/{job_id_str}"
