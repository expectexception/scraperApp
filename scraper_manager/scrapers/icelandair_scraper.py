...
class IcelandairScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.api_url = (
            "https://static-jobs-api.50skills.app/public/icelandair/jobs.json"
        )
    ...
    async def fetch_jobs(self) -> list:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"[{self.site_key}] Failed to fetch JSON API: {response.status_code}"
                )
                return []
            ...
            for item in data:
                ...
                try:
                    ...
                    location = (
                        content.get("location") or item.get("locationName") or "Various"
                    )
