        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for i, job_data in enumerate(page_jobs_data):
                    ...
                    try:
                        logger.info(
                            f"[{self.site_key}] Fetching details for: {job_data['title']}"
                        )
                        ...
                        if i < len(rows):
                            ...
                            if not title_btn:
                                title_btn = await rows[i].query_selector(
                                    ".current-opening-title"
                                )
                            ...
                            if title_btn:
                                ...
                                try:
                                    await page.wait_for_selector(
                                        ".job-description-requisition, .job-description-data-item",
                                        timeout=15000,
                                    )
                                except:
                                    logger.warning(
                                        f"[{self.site_key}] Timeout waiting for details for {job_data['title']}"
                                    )

                                # Get Req ID
                                req_el = await page.query_selector(
                                    ".job-description-requisition"
                                )
                                req_text = await req_el.inner_text() if req_el else ""
                                job_id = (
                                    re.search(r"(\d+)", req_text).group(1)
                                    if re.search(r"(\d+)", req_text)
                                    else str(hash(job_data["title"]))
                                )

                                # Get Description
                                desc_el = await page.query_selector(
                                    ".job-description-data-item"
                                )
                                description = (
                                    await desc_el.inner_text() if desc_el else ""
                                )
                                ...
                                back_btn = await page.query_selector(
                                    'button[aria-label="Back to Search Results"], .back-to-search-results'
                                )
                                if back_btn:
                        ...
                        logger.error(
                            f"[{self.site_key}] Error fetching job detail for {job_data['title']}: {e}"
                        )
