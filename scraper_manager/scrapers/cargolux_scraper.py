    ...
    async def fetch_jobs(self) -> list:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                search_btn = await page.query_selector('#sp-searchButton, input[type="button"][value="Search"]')
                if search_btn:
                    logger.info(f"[{self.site_key}] Clicking Search button to load all jobs...")
                    await search_btn.click()
                    # Wait for results to appear
                    try:
                        await page.wait_for_selector('.pf-srp-singlehit', timeout=15000)
                    except:
                        logger.warning(f"[{self.site_key}] Timeout waiting for .pf-srp-singlehit after click.")
                else:
                    logger.warning(f"[{self.site_key}] Search button not found, attempting to wait for results anyway.")
                ...
                while True:
                    ...
                    # Wait for results
                    try:
                        await page.wait_for_selector('.pf-srp-singlehit, .pf-srp-jobResult', timeout=15000)
                    except:
                        logger.warning(f"[{self.site_key}] No jobs found on page {page_count}")
                        break

                    job_rows = await page.query_selector_all('.pf-srp-singlehit, .pf-srp-jobResult')
                    if not job_rows:
                        break
                    ...
                    for row in job_rows:
                        try:
                            title_el = await row.query_selector('a.pf-srp-jobResult-title, .pf-srp-jobResult-title a')
                            if not title_el:
                                continue
                    ...
                    for job_data in page_jobs_data:
                        ...
                        try:
                            ...
                            desc_el = await detail_page.query_selector('.pf-job-post-content, #pf-job-description, .pf-description')
                            description = ""
                            if desc_el:
                    ...
                    # Pagination
                    try:
                        next_btn = await page.query_selector('a[title="Next"], .pf-srp-pagination-next')
                        if next_btn:
