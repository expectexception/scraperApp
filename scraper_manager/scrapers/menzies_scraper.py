            ...
            try:
                ...
                try:
                    cookie_btn = await page.query_selector(
                        'button:has-text("Accept"), .cookie-accept, #accept-cookies'
                    )
                    if cookie_btn:
                ...
                for p_num in itertools.count(1):
                    ...
                    for el in elements:
                        try:
                            ...
                            if parent:
                                ...
                                loc_el = await page.evaluate(
                                    '(parent) => { let text = parent.innerText || ""; return text.split("\\n").find(t => t.includes(",") || t.length > 5 && !t.includes("Closing Date")) || "Global"; }',
                                    parent,
                                )
                                if loc_el:
                            ...
                            print(f"DEBUG: Appended job {title}")

                            if self.max_jobs and len(jobs) >= self.max_jobs:
                                break
                        except Exception as ex:
                            print(f"DEBUG: Exception extracting job: {ex}")
                            continue
                    ...
                    next_btn = await page.query_selector(
                        "a.scroller_movenext.buttonEnabled"
                    )
                    if next_btn:
