...
class FlydubaiScraper(BaseScraper):
    ...
    def __init__(self, config, db_manager=None):
        ...
        self.jobs_url = self.site_config.get('jobs_url', 'https://careers.flydubai.com/jobs')
    ...
    async def fetch_jobs_from_listing(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                logger.info("Loading Flydubai careers page (Angular - requires long wait)...")
                await self.random_delay(2, 4)
                await page.goto('https://careers.flydubai.com/jobs', wait_until='networkidle', timeout=90000)
                ...
                logger.info(f"Found {len(job_links)} jobs using verified selector: a.job-title-link")

                if not job_links:
                    logger.warning("No job links found with verified selector")

                if not job_links:
                    logger.warning("No job links found with known selectors, falling back to all generic links")
                    # Generic fallback
                ...
                for link in job_links:
                    ...
                    try:
                        ...
                        skip_words = {'apply now', 'log in', 'login', 'careers', 'home', 'search', 'sign up', 'register', 'all open positions'}
                        ...
                        parent_handle = await link.evaluate_handle('el => el.closest("tr") || el.closest("li") || el.closest(".job-card")')
                        parent = parent_handle.as_element()
                        if parent:
                            loc_elem = await parent.query_selector('.location, [class*="location"]')
                            if loc_elem:
                                loc_text = await loc_elem.inner_text()
                                job_data['location'] = loc_text.strip()
                            
                            date_elem = await parent.query_selector('.date, [class*="date"]')
                            if date_elem:
                                date_text = await date_elem.inner_text()
                                parsed = self.parse_posted_date(date_text)
                                job_data['posted_date'] = parsed if parsed else date_text.strip()
                                
                            dept_elem = await parent.query_selector('.department, [class*="department"]')
                            if dept_elem:
