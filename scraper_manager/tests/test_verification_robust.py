...
class JobVerificationRobustTests(TestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_user(username='admin_tester', password='pass')
    ...
    def test_keyword_closure_detection(self):
        ...
        test_cases = [
            ("This position has been filled", False, "closed_keyword:position has been filled"),
            ("The role has already been filled", False, "closed_keyword:has already been filled"),
            ("This vacancy is no longer being advertised", False, "closed_keyword:is no longer being advertised"),
            ("This job is no longer available", False, "closed_keyword:this job is no longer available"),
        ...
        for body, expected_active, reason_snippet in test_cases:
            ...
            with mock.patch('curl_cffi.requests.get', return_value=mock_resp):
                is_active, reason = async_to_sync(self.db_manager.check_job_active)("http://example.com/job")
                self.assertEqual(is_active, expected_active, f"Failed for body: {body[:30]}...")
    ...
    def test_management_command_filtering(self):
        ...
        with mock.patch('scraper_manager.db_manager.DjangoDBManager.check_job_active', return_value=(False, "mock_closed")):
