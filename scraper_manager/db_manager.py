...
class DjangoDBManager:
    ...
    def _infer_operation_type(self, title: str, company: str, description: str = '') -> Optional[str]:
        ...
        company_priorities = {
            'cargo': [
                ...
                'blue dart', 'silk way', 'cargojet', 'asl airlines', 'martinair'
            ],
            'mro': [
                'haeco', 'gameco', 'lufthansa technik', 'air france industries', 'klm e&m',
                'st engineering', 'aarf', 'sia engineering', 'mtu aero', 'sr technics',
                'signature aviation', 'standardaero', 'aviall', 'jet aviation', 'shannon engine'
            ],
            'business': [
                'netjets', 'flexjet', 'vista', 'xo', 'wheels up', 'flyexclusive',
                'tag aviation', 'execujet', 'air charter service', 'clay lacy',
                'solairus', 'privatefly', 'air partner', 'jetcraft'
            ],
            'helicopter': [
                'phi', 'air methods', 'bristow', 'era helicopters', 'chn', 'cougar helicopters',
                'babcock mission', 'airbus helicopters', 'sikorsky', 'bell flight'
            ]
        }
        ...
        patterns = {
            'cargo': {
                'keywords': ['cargo', 'freight', 'logistics', 'supply chain', 'warehouse', 'loadmaster', 'acm', 'main-deck', 'pallet'],
                'phrases': ['cargo operations', 'freight forwarder', 'cargo agent', 'main deck', 'cargo handler']
            },
            'mro': {
                'keywords': ['mro', 'maintenance', 'repair', 'overhaul', 'mechanic', 'technician', 'avionics', 'sheet metal', 'a&p', 'license', 'b1', 'b2', 'base maintenance', 'line maintenance', 'aog', 'camu'],
                'phrases': ['aircraft maintenance', 'engine repair', 'component overhaul', 'heavy maintenance', 'technical records', 'airworthiness engineer']
            },
            'business': {
                'keywords': ['bizjet', 'private jet', 'vip', 'vvip', 'charter', 'corporate', 'fractional', 'fbo', 'signature', 'signature-aviation', 'atlantic aviation'],
                'phrases': ['business aviation', 'executive jet', 'private aviation', 'corporate flight', 'vip cabin']
            },
            'helicopter': {
                'keywords': ['helicopter', 'rotor', 'rotary', 'vtol', 'hems', 'offshore', 'heli-deck'],
                'phrases': ['rotary wing', 'helicopter pilot', 'rotorcraft maintenance']
            },
            'ground_ops': {
                'keywords': ['ramp', 'baggage', 'gate', 'station', 'dispatcher', 'fueler', 'lounge', 'concierge', 'screener', 'aisats', 'celebi', 'menzies', 'bird group'],
                'phrases': ['ground handling', 'airport operations', 'passenger service', 'ramp agent', 'turnaround coordinator']
            },
            'atc': {
        ...
        low_cost_carriers = [
            ...
            'flydubai', 'air arabia', 'pegasus', 'cebu pacific', 'akasa'
        ]
        scheduled_carriers = [
            'american airlines', 'delta', 'united', 'british airways', 'lufthansa',
            'air france', 'klm', 'emirates', 'qatar', 'etihad', 'singapore', 
            'cathay', 'ana', 'jal', 'qantas', 'air india', 'vistar'
        ]
    ...
    def _calculate_deduplication_hash(self, title: str, company: str, location: str) -> str:
        """
    ...
    def _save_to_jobs_model(self, job_data: Dict, source: str):
        ...
        try:
            ...
            mapping = CompanyMapping.objects.filter(normalized_name=norm_company).first()
            if mapping:
            ...
            dedup_hash = self._calculate_deduplication_hash(title, mapped_company, location)
            ...
            # Only set posted_date if we actually parsed one
            ...
            if not job:
                ...
                job = Job.objects.filter(status='active').filter(title__iexact=title, company__iexact=mapped_company, location__iexact=location).first()
            ...
            if job:
                ...
                if job.url == url:
                    ...
                    logger.info(f"[{source}] Deduplicated job (cross-source): {title} already exists from {job.source}")
                    job_created = False
            else:
    ...
    def add_jobs_batch(self, jobs: List[Dict], source: str) -> Dict[str, int]:
        ...
        companies = {job.get('company', '').strip().lower() for job in jobs if job.get('company')}
        mappings = {m.normalized_name: m for m in CompanyMapping.objects.filter(normalized_name__in=companies)}
    ...
    async def check_job_active(self, url: str, job_obj: Optional[Job] = None, use_playwright: bool = False) -> tuple[bool, str]:
        """
        ...
        try:
            ...
            if job_obj and job_obj.raw_json:
                ...
                expiry_keys = [
                    'valid_through', 'validThrough', 'expiration_date', 
                    'expiry_date', 'expires_at', 'closing_date', 'deadline'
                ]
            ...
            if len(r.history) > 0:
                ...
                generic_paths = [
                    '', 'jobs', 'careers', 'opportunities', 'vacancies', 'positions',
                    'en-us', 'search-results', 'all-jobs', 'job-search', 'job-not-found',
                    'job-expired', 'application-closed', 'job-closed'
                ]
                ...
                if parsed_final.netloc != parsed_original.netloc:
                    ...
                    known_patterns = ['jobs.', 'career.', 'careers.', 'recruitment.', 'hiring.', 'talent.']
                    if not any(known in parsed_final.netloc for known in known_patterns):
                        # Some companies redirect to a global portal, but generic ones are usually bad
                        if any(generic in parsed_final.netloc for generic in ['workday', 'icims', 'lever', 'greenhouse']):
            ...
            if 'community.workday.com/invalid-url' in body_lower or 'window.location.href' in body_lower and 'invalid' in body_lower:
            ...
            for pat in redirect_patterns:
                ...
                if match:
                    ...
                    if any(x in redir_target.lower() for x in ['login', 'expired', 'closed', 'notfound']):
            ...
            for match in re.findall(r'<script[^>]*type=["\']application/ld\+json["\'][^>]*>(.*?)</script>', json_ld_section, re.DOTALL | re.IGNORECASE):
                try:
                    ...
                    async with async_playwright() as p:
                        ...
                        try:
                            ...
                            if not resp or resp.status in (404, 410):
                                return False, f'pw_http_{resp.status if resp else "no_resp"}'
                            
                            # Final state check
                            final_url = page.url
                            if any(x in final_url.lower() for x in ['expired', 'closed', 'job-not-found']):
    ...
    async def verify_active_jobs(self, limit: int = 100):
        ...
        active_jobs = Job.objects.filter(status='active').order_by('last_checked')[:limit]
        ...
        for job in active_jobs:
            ...
            if not is_active:
                ...
                stats['closed'] += 1
                logger.info(f"Marked job closed during verification: {job.title} ({reason})")
            else:
                job.last_checked = timezone.now()
