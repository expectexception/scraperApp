    ...
    def get_metrics(cls, operation: Optional[str] = None) -> Dict:
        ...
        if operation:
            ...
            return {
                ...
                "error_rate": round((metric["errors"] / metric["count"] * 100), 1)
                if metric["count"] > 0
                else 0,
...
class ProfileOperation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        PerformanceMetrics.record(
            self.operation_name, self.duration, success=success, **self.metadata
        )
        ...
        if success:
            log_func(
                f"{status} Completed: {self.operation_name} in {self.duration:.3f}s"
            )
        else:
            log_func(
                f"{status} Failed: {self.operation_name} after {self.duration:.3f}s - {exc_type.__name__}"
            )
