...
class BatchProcessor:
    ...
    def flush_scraped_urls(self) -> Tuple[int, int]:
        ...
        try:
            ...
            if urls_to_create:
                ScrapedURL.objects.bulk_create(
                    urls_to_create, batch_size=self.batch_size
                )
            ...
            if urls_to_update:
                ScrapedURL.objects.filter(url__in=urls_to_update).update(
                    scrape_count=ScrapedURL.objects.filter(
                        url__in=urls_to_update
                    ).values("scrape_count")[0]["scrape_count"]
                    + 1,
    ...
    def flush_jobs(self) -> Tuple[int, int]:
        ...
        try:
            urls_batch = [
                item["job_data"].get("url", "").strip() for item in self.jobs_buffer
            ]

            # Get existing URLs
            existing_jobs = set(
                Job.objects.filter(url__in=urls_batch).values_list("url", flat=True)
            )
