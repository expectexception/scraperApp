...
class JobFilterManager:
        ...
        try:
            ...
            for filter_group in self.filters:
                ...
                for keyword in keywords:
                    ...
                    pattern = re.compile(
                        r"\b" + re.escape(keyword_lower) + r"\b", re.IGNORECASE
                    )
            ...
            logger.info(f"✓ Loaded {len(self.filters)} filter categories")
            logger.info(
                f"✓ Total keywords: {len(self.all_keywords)} ({len(self.phrase_keywords)} phrases, {len(self.single_keywords)} single words)"
            )
            logger.info(
                f"✓ Patterns precompiled: {len(self.phrase_patterns) + len(self.single_patterns) + len(self.exclusion_compiled)}"
            )

        except Exception as e:
            logger.info(f"❌ Error loading filters: {e}")
    ...
    def _matches_filter_impl(self, title_lower: str) -> Tuple[bool, Tuple, float, Dict]:
            ...
            if pattern.search(title_lower):
                ...
                for cat in categories:
                    ...
                    category_scores[cat_name] = (
                        category_scores.get(cat_name, 0.0) + score_add
                    )
        ...
        for pattern, keyword in self.single_patterns:
            if pattern.search(title_lower):
                ...
                for cat in categories:
                    ...
                    cat_name = cat["display_name"]
                    category_scores[cat_name] = (
                        category_scores.get(cat_name, 0.0) + effective_weight
                    )
        ...
        match_details = {
            ...
            if any(" " in k for k in matched_keywords)
            else "single_match",
        }
    ...
    def filter_jobs(self, jobs: List[Dict]) -> Tuple[List[Dict], List[Dict], Dict]:
                ...
                job.update(
                    {
                        ...
                        "matched_filter_types": [
                            cat["filter_type"] for cat in categories
                        ],
                        "matched_keywords": details.get("matched_keywords", []),
                        "category_scores": details.get("category_scores", {}),
                        "primary_category": max(
                            details["category_scores"].items(), key=lambda x: x[1]
                        )[0]
        ...
        stats["performance"] = {
            ...
            "jobs_per_second": round(len(jobs) / (elapsed_ms / 1000), 1)
            if elapsed_ms > 0
            else 0,
            ...
            if (self.perf_stats["cache_hits"] + self.perf_stats["cache_misses"]) > 0
            else 0,
        }
    ...
    def print_filter_stats(self, stats: Dict):
        ...
        logger.info(f"✅ Matched (will scrape): {stats['matched']}")
        logger.info(f"❌ Rejected (skipped): {stats['rejected']}")

        if stats.get("excluded", 0) > 0:
            logger.info(f"🚫 Excluded by pattern: {stats['excluded']}")
        ...
        if stats["matched"] > 0:
            ...
            if any(score_dist.values()):
                logger.info("\n🎯 Score Distribution:")
                logger.info(
                    f"  • High confidence (≥5.0):    {score_dist.get('high', 0)} jobs"
                )
                logger.info(
                    f"  • Medium confidence (≥3.0):  {score_dist.get('medium', 0)} jobs"
                )
                logger.info(
                    f"  • Low confidence (≥1.5):     {score_dist.get('low', 0)} jobs"
                )
        ...
        if perf:
            ...
            logger.info(f"  • Per-job avg: {perf.get('avg_per_job_ms', 0):.3f}ms")
            if perf.get("cache_efficiency", 0) > 0:
                logger.info(
                    f"  • Cache efficiency: {perf.get('cache_efficiency', 0):.1f}% hit rate"
                )
    ...
    def get_filter_summary(self) -> Dict:
        ...
        summary = {
            ...
            "precompiled_patterns": len(self.phrase_patterns)
            + len(self.single_patterns),
    ...
    def print_filter_info(self):
        ...
        logger.info(f"\n{'=' * 70}")
        logger.info("🔍 FILTER CONFIGURATION & OPTIMIZATIONS")
        logger.info(f"{'=' * 70}")
        ...
        logger.info(f"  • Phrases (multi-word): {summary['phrase_keywords']}")
        logger.info(f"  • Single words: {summary['single_keywords']}")

        logger.info("\n⚡ Performance Optimizations:")
        logger.info(
            f"  • Precompiled regex patterns: {summary['precompiled_patterns']}"
        )
        logger.info(
            f"  • Precompiled exclusion patterns: {summary['exclusion_patterns']}"
        )
        logger.info(f"  • Result caching enabled: {summary['cache_enabled']}")
        logger.info("  • Cache size: 10,000 entries (LRU)")
        logger.info("  • Matching strategy: 2-phase (phrases→single words)")
        logger.info("  • Early exit on high confidence (≥4.0)")

        logger.info("\nCategories:")
        for cat in summary["categories"]:
            logger.info(f"\n  📁 {cat['name']}")
            logger.info(f"     Type: {cat['type']}")
            logger.info(f"     Keywords: {cat['keyword_count']}")
...
if __name__ == "__main__":
    ...
    logger.info("\n" + "=" * 70)
    logger.info("TEST: Optimized Matching with Performance Tracking")
    logger.info("=" * 70)
    ...
    for title in test_titles:
        ...
        if matches:
            ...
            primary = (
                max(details["category_scores"].items(), key=lambda x: x[1])[0]
            ...
            logger.info(f"   All Categories: {', '.join(cat_names)}")
            logger.info(
                f"   Keywords: {', '.join(details['matched_keywords'][:5])}"
            )  # Show first 5
        else:
    ...
    logger.info(f"Avg time per job: {cache_stats['avg_filter_time_ms']}ms")
